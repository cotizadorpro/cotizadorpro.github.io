...
// ── [v1.11.63] ALERTAS DE VIGENCIA ──────────────────────────────────────────
// Muestra equipos vencidos y equipos que vencen hoy o en los próximos 3 días.
// El punto rojo del menú se enciende mientras exista al menos una alerta.
function _vigPuedeVer(){
  return (typeof _meEsSuper === 'function') && _meEsSuper();
}
function vigenciasResumen(){
  var all=CAT.ios.concat(CAT.android), porVencer=[], vencidos=[];
  all.forEach(function(d){
    var dias=daysLeft(d.id);
    if(dias===999) return;
    var row={id:d.id,name:d.name,fecha:VIGENCY[d.id],dias:dias};
  ...
  vencidos.sort(function(a,b){return b.dias-a.dias||(a.name<b.name?-1:1);});
  return {porVencer:porVencer,vencidos:vencidos};
}
function updateVigenciasCard(){
  ...
  if(puede){
    try{
      ...
      n=r.porVencer.length+r.vencidos.length;
    }catch(e){n=0;}
  }
...
function openVigencias(){
  ...
  var r=vigenciasResumen();
  var fdia=function(n){return n===0?'vence hoy':(n===1?'vence en 1 día':'vence en '+n+' días');};
  ...
  var body='';
  body+=bloque('Equipos vencidos',r.vencidos,'#FF3B30',function(it){
    return '<div style="font-size:12px;font-weight:600;color:#FF3B30">'+fvenc(it.dias)+'</div>';
  });
  body+=bloque('Próximos a vencer',r.porVencer,'#FF9500',function(it){
    var c=it.dias<=1?'#FF3B30':'#FF9500';
    return '<div style="font-size:12px;font-weight:600;color:'+c+'">'+fdia(it.dias)+'</div>';
  });
  if(!body)body='<div style="text-align:center;padding:26px 10px;color:var(--label3);font-size:13px">Todo en orden ✓<br><span style="font-size:11px">No hay equipos vencidos ni próximos a vencer.</span></div>';

  modal.innerHTML=
    '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">'
   +'<div style="font-size:18px;font-weight:700">Vigencias</div>'
   +'<button id="vig-close" style="background:none;border:none;font-size:22px;cursor:pointer;color:var(--label3);padding:0 4px;line-height:1">✕</button></div>'
   ...
   +'Aquí puedes identificar los <b>equipos cuya vigencia ya terminó</b> y los que <b>vencen en los próximos 3 días</b>. El punto rojo del menú indica que hay vigencias que requieren atención.'
   +'</div>'+body
  ...
  ov.appendChild(modal);document.body.appendChild(ov);document.body.style.overflow='hidden';
...
function show(id){
  // [V53] Actividad es una vista temporal bajo el menú global.
  // Al navegar a cualquier pantalla normal, retirarla por completo para que
  // nunca quede superpuesta sobre Inicio, Catálogo, Accesos, etc.
  const _dashOv=document.getElementById('dash-overlay');
  if(_dashOv) _dashOv.classList.remove('show');
...
function tCmp(id){
  ...
  else if(cmpSet.size<2) cmpSet.add(id);
  renderDevs();
}
function clearCmp(){cmpSet.clear();renderDevs()}
function updateCmpBar(){
  const bar=document.getElementById('cmp-bar');
  document.getElementById('cmp-lbl').textContent=`${cmpSet.size} seleccionado${cmpSet.size!==1?'s':''} de 2`;
...
function toggleFichaCmp(){
  ...
  else if(cmpSet.size<2)cmpSet.add(curFichaId);
  const btn=document.getElementById('f-cmp-btn');
  btn.textContent=cmpSet.has(curFichaId)?'✓ Características':'+ Características';
...
function showFicha(id){
  ...
  const btn=document.getElementById('f-cmp-btn');
  btn.textContent=cmpSet.has(id)?'✓ Características':'+ Características';
  ...
  // Hero
  h+='<div class="ficha-hero"><div class="fh-inner" style="position:relative">';
  h+='<div id="cot-main-device-compare-actions" style="position:absolute;right:14px;top:8px;z-index:20;display:flex;gap:6px;align-items:center">';
  h+='<button id="cot-main-device-compare-btn" type="button" onclick="compararEquipoActual()" style="height:32px;padding:0 14px;border-radius:8px;border:1px solid rgba(255,255,255,.14);background:#59616A;color:#fff;font-size:10px;font-weight:700;letter-spacing:.25px;cursor:pointer">'+((cotEquipoComparar&&cotEquipoComparar.device)?'GENERAR COMPARATIVA':'COMPARAR EQUIPOS')+'</button>';
  h+='<button id="cot-main-device-compare-cancel" type="button" onclick="cancelarComparativaEquipos(event)" title="Cancelar comparativa" style="'+((cotEquipoComparar&&cotEquipoComparar.device)?'':'display:none;')+'width:32px;height:32px;border-radius:8px;border:1px solid rgba(255,255,255,.14);background:#59616A;color:#fff;font-size:18px;cursor:pointer">×</button>';
  h+='</div>';
...
let _variants=(window.STORAGE_VARIANTS || {})[id] || null;
  ...
  h+='<div class="tb" onclick="swTab(\'specs\',this)">Características</div>';
  ...
  h+='<button onclick="enviarCotizacion()" class="wa-btn" style="width:100%;padding:13px 18px;background:#00A8E0;color:#fff;font-size:14px;font-weight:600;border:none;border-radius:12px;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;letter-spacing:.8px;font-family:Roboto,sans-serif;box-shadow:0 6px 16px rgba(0,168,224,.18);transition:transform .16s ease,filter .16s ease">';
...
function _cmpNormKey(k){
  return String(k||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'')
    .toLowerCase().replace(/[^a-z0-9]+/g,' ').trim();
}
function _cmpData(){
  const all=[...CAT.ios,...CAT.android];
  return [...cmpSet].map(id=>all.find(d=>d.id===id)).filter(Boolean).slice(0,2);
}
function _cmpRows(devs){
  const rows=[], seen={};
  devs.forEach(function(d){
    Object.keys(d.specs||{}).forEach(function(k){
      const nk=_cmpNormKey(k);
      if(!nk || seen[nk]) return;
      seen[nk]=true;
      rows.push({key:k,norm:nk});
    });
  });
  return rows;
}
function _cmpSpecValue(d,row){
  const specs=d.specs||{};
  if(Object.prototype.hasOwnProperty.call(specs,row.key)) return specs[row.key];
  const found=Object.keys(specs).find(k=>_cmpNormKey(k)===row.norm);
  return found ? specs[found] : '—';
}
function _buildFeatureCompareHTML(devs,forImage){
  const rows=_cmpRows(devs);
  const headers=devs.map(function(d){
    const img=IMG[d.id]
      ? '<div class="fc-device-imgbox"><img class="fc-device-img" src="'+_cmpEsc(IMG[d.id])+'" crossorigin="anonymous" alt=""></div>'
      : '<div class="fc-device-imgbox"></div>';
    return '<div class="fc-device">'+img+
      '<div class="fc-device-brand">'+_cmpEsc(d.brand)+'</div>'+
      '<div class="fc-device-name">'+_cmpEsc(d.name)+'</div>'+
      '<div class="fc-device-storage">'+_cmpEsc(d.storage||'')+'</div></div>';
  }).join('');

  const specRows=rows.map(function(r){
    return '<div class="fc-row"><div class="fc-key">'+_cmpEsc(r.key)+'</div>'+
      devs.map(d=>'<div class="fc-val">'+_cmpEsc(_cmpSpecValue(d,r))+'</div>').join('')+
      '</div>';
  }).join('');

  const sell=devs.map(function(d){
    const arr=Array.isArray(d.sell)?d.sell.slice(0,3):[];
    return '<div class="fc-sell-col"><div class="fc-sell-name">'+_cmpEsc(d.name)+'</div>'+
      (arr.length?arr.map(s=>'<div class="fc-sell-item">✓ '+_cmpEsc(s)+'</div>').join(''):
      '<div class="fc-sell-item">Sin argumentos registrados</div>')+'</div>';
  }).join('');

  return '<div class="feature-compare-card'+(forImage?' feature-compare-export':'')+'">'+
    '<div class="fc-title">Comparativa de características</div>'+
    '<div class="fc-sub">Consulta las diferencias principales entre ambos equipos</div>'+
    '<div class="fc-devices"><div class="fc-key fc-key-head"></div>'+headers+'</div>'+
    '<div class="fc-specs">'+specRows+'</div>'+
    '<div class="fc-sell-title">Argumentos clave de venta</div>'+
    '<div class="fc-sell-grid">'+sell+'</div>'+
    '</div>';
}
let featureCompareReturnScreen='s-home';
function volverComparativaCaracteristicas(){
  show(featureCompareReturnScreen||'s-home');
}
window.volverComparativaCaracteristicas=volverComparativaCaracteristicas;
...
function goCompare(){
  ...
  body.innerHTML=_buildFeatureCompareHTML(devs,false);
  const active=document.querySelector('.screen.active');
  if(active && active.id && active.id!=='s-compare') featureCompareReturnScreen=active.id;
  show('s-compare');
}
async function compartirComparativaCaracteristicas(){
  const devs=_cmpData();
  if(devs.length<2)return;
  if(typeof ensureVendors==='function') await ensureVendors();
  if(typeof html2canvas==='undefined'){
    alert('No se pudo cargar el generador de imagen.');
    return;
  }
  const host=document.createElement('div');
  host.className='feature-compare-render-host';
  host.innerHTML=_buildFeatureCompareHTML(devs,true);
  document.body.appendChild(host);
  try{
    const card=host.querySelector('.feature-compare-card');
    const canvas=await html2canvas(card,{
      scale:2,useCORS:true,backgroundColor:'#EEF1F4',logging:false
    });
    const blob=await new Promise(resolve=>canvas.toBlob(resolve,'image/png',0.96));
    if(!blob) throw new Error('No se pudo crear PNG');
    const file=new File([blob],'comparativa-caracteristicas.png',{type:'image/png'});
    if(navigator.share && (!navigator.canShare || navigator.canShare({files:[file]}))){
      try{
        await navigator.share({files:[file],title:'Comparativa de características'});
        return;
      }catch(e){
    ...
    a.download='comparativa-caracteristicas.png';
    document.body.appendChild(a);a.click();a.remove();
    setTimeout(()=>URL.revokeObjectURL(a.href),1500);
  }catch(e){
    console.error('[feature-compare]',e);
    alert('No se pudo generar la imagen de la comparativa.');
  }finally{
    host.remove();
  }
}

window.featureCompareSelectPair=function(firstId,secondId){
  cmpSet.clear();
  if(firstId)cmpSet.add(firstId);
  if(secondId)cmpSet.add(secondId);
  if(cmpSet.size===2)goCompare();
};
window.featureCompareStart=function(firstId){
  cmpSet.clear();
  if(firstId)cmpSet.add(firstId);
};
window.featureCompareCancel=function(){
  cmpSet.clear();
  try{updateCmpBar();}catch(_e){}
};
...
// [v1.11.69] COMPARATIVA DE EQUIPOS — flujo limpio.
// Opción 1 y Opción 2 son snapshots del mismo estado que usa openCotModal().
function _actualizarBotonComparativa(){
  const b=document.getElementById('cot-main-device-compare-btn');
  const x=document.getElementById('cot-main-device-compare-cancel');
  if(!b) return;
  if(cotEquipoComparar && cotEquipoComparar.device){
    b.textContent='GENERAR COMPARATIVA';
    if(x) x.style.display='';
  }else{
    b.textContent='COMPARAR EQUIPOS';
    if(x) x.style.display='none';
  }
}

async function compararEquipoActual(){
  const actual=_crearCotStateActual();
  if(!actual) return;
  const fase=(cotEquipoComparar&&cotEquipoComparar.device)?2:1;
  openDeviceCompareConfigurator(actual,fase,function(configured){
    if(fase===1){
      cotEquipoComparar=JSON.parse(JSON.stringify(configured));
      _actualizarBotonComparativa();
      alert('Opción 1 guardada: '+configured.device.name+'. Ahora selecciona y configura el segundo equipo.');
    }else{
      if(cotEquipoComparar.device.id===configured.device.id &&
         cotEquipoComparar.plan===configured.plan &&
         String(cotEquipoComparar.plazo)===String(configured.plazo)){
        alert('La Opción 2 debe ser otro equipo o una configuración diferente.'); return;
      }
      cotState=JSON.parse(JSON.stringify(configured));
      generateFlyerImage();
    }
  });
}

function cancelarComparativaEquipos(ev){
  if(ev){ ev.preventDefault(); ev.stopPropagation(); }
  cotEquipoComparar=null;
  _actualizarBotonComparativa();
}
...
// [v1.11.64] Comparativa de 2 equipos completos.
// Guarda una fotografía independiente de la primera cotización mientras
// el asesor configura el segundo equipo.
let cotEquipoComparar = null;
...
// [v1.11.68] FUENTE ÚNICA DEL ESTADO ACTUAL DEL COTIZADOR.
// Generar cotización y Comparar equipos usan exactamente esta misma función.
function _crearCotStateActual(){
  ...
  const promo=resolvePrice(curFichaId, curPlan, curPlazo);
  if(promo===null||promo===undefined){alert('No hay precio para esta combinación');return null;}
...
function openCotModal(){
  const state=_crearCotStateActual();
  if(!state) return;
  cotState=state;
  ...
  let _subtitle = state.device.name + ' · ' + state.plan + ' · ' + state.plazo + ' meses';
  ...
  // Build rentas pills (0,1,2,3) + [v1.8.2] botón "Garantía"
  ...
  rentasHtml += '<button class="cot-pill cot-pill-custom" onclick="cotSetDeposito()" id="cot-deposito-pill">Garantía</button>';
...
const controlPrice = cotState.control ? 50 : 0;
  ...
  // [v1.8.2] Garantía (se suma al pago inicial, NO disminuye mensualidad)
  if(depositoGarantia > 0){
    h += '<div class="cot-resumen-row"><span class="cot-resumen-label">Garantía</span><span class="cot-resumen-val">$'+fmx(depositoGarantia)+'</span></div>';
  }
  
  // Total inicial
  if(totalInicial>0){
    h += '<div class="cot-resumen-row cot-resumen-pago-inicial" style="background:rgba(0,122,255,.06);margin:6px 0 0;padding:8px 0;border-radius:8px"><span class="cot-resumen-label" style="font-weight:700">Pago inicial</span><span class="cot-resumen-val" style="color:var(--ios-blue);font-size:16px">$'+fmx(totalInicial)+'</span></div>';
  }
...
function cotSend(){
  ...
  const engPay = cotCalcEngancheAmount();
  const rentasGarantia = cotState.rentas * cotState.planRenta;
  // [v1.8.3] Garantía: suma al pago inicial
  const depositoGarantia = cotState.deposito || 0;
  const totalInicial = engPay + rentasGarantia + depositoGarantia;
...
const totalMensualSinPort =
 ...
 if(cotState.port){
  ...
  if(depositoGarantia > 0) _conceptosIniciales.push('Garantía $' + fmx(depositoGarantia));
  ...
  if(navigator.share){
    try{
      await navigator.share({
        title: 'Cotización Cotizador Pro',
        text: msg
      });
...
function showProductInfo(sku){
  ...
  h += '<div style="font-size:10px;color:var(--label3);font-family:Roboto,sans-serif;margin-top:4px">SKU: '+acc.sku+'</div>';
  h += '</div>';
...
function buildSpecsPDFHTML(dev){
  const productImg = (typeof IMG !== 'undefined' && IMG[dev.id])
    ? '<img src="'+IMG[dev.id]+'" alt="" style="max-width:150px;max-height:190px;object-fit:contain;display:block;margin:0 auto" crossorigin="anonymous">'
    : '<div style="font-size:72px;text-align:center;opacity:0.25">📱</div>';
  ...
  return '<div class="specs-page" style="width:794px;height:1123px;overflow:hidden;background:#FFFFFF;font-family:\'Roboto\',sans-serif;color:#26313D;line-height:1.35;box-sizing:border-box">'+
    // ── Header: SE CONSERVA EXACTAMENTE EL DE v1 ─────────────────────────
    '<div style="display:flex;align-items:center;justify-content:space-between;padding:24px 42px;background:#50565E;border-bottom:1px solid rgba(255,255,255,.12);color:#fff">'+
      '<div style="display:flex;align-items:center;gap:9px;min-width:0">'+
        '<svg width="25" height="31" viewBox="0 0 24 24" fill="none" stroke="#00A8E0" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" style="display:block;flex:0 0 auto"><rect x="6" y="2" width="12" height="20" rx="2.5"></rect><path d="M10 5h4M11 19h2"></path></svg>'+
        '<div style="font-size:26px;line-height:1;font-weight:700;white-space:nowrap"><span style="color:#00A8E0">Cotizador</span> <span style="color:#E7E9EC;font-weight:400">Pro</span></div>'+
      '</div>'+
      '<div style="font-size:11px;color:#E7E9EC;letter-spacing:.1em;font-weight:600">FICHA TÉCNICA · '+dateStr+'</div>'+
    '</div>'+

    // ── Producto: sencillo y compacto ────────────────────────────────────
    '<div style="padding:22px 42px 16px;text-align:center">'+
      '<div style="background:#fff;border:1px solid #ECEFF2;border-radius:14px;padding:14px;margin:0 auto 13px;width:220px;height:210px;box-sizing:border-box;display:flex;align-items:center;justify-content:center">'+
        productImg+
      '</div>'+
      '<div style="font-size:11px;color:#7A8087;text-transform:uppercase;letter-spacing:.12em;font-weight:600;margin-bottom:4px">'+(dev.brand||'')+'</div>'+
      '<div style="font-size:27px;font-weight:600;color:#252B31;letter-spacing:-.02em;margin-bottom:3px;line-height:1.1">'+(dev.name||'')+'</div>'+
      '<div style="font-size:14px;color:#60666D;font-weight:500">'+(dev.storage||'')+(dev.bundle?' · '+dev.bundle:'')+'</div>'+
    '</div>'+

    // ── Especificaciones: gris claro, aireadas, sin iconos ───────────────
    '<div class="specs-wrap">'+
      '<div class="specs-title">Características</div>'+
      '<div class="specs-card"><table>'+specsHTML+'</table></div>'+
    '</div>'+

    // ── Margen inferior deliberado: evita cualquier corte visual ─────────
    '<div class="specs-bottom-space"></div>'+

    '<style>'+
      '.specs-page .specs-wrap{padding:0 42px 0;box-sizing:border-box}'+
      '.specs-page .specs-title{font-size:11px;font-weight:700;color:#50565E;text-transform:uppercase;letter-spacing:.12em;margin:0 0 10px}'+
      '.specs-page .specs-card{background:#F1F3F5;border:1px solid #E1E5E8;border-radius:12px;overflow:hidden}'+
      '.specs-page table{width:100%;border-collapse:collapse;table-layout:fixed}'+
      '.specs-page table tr{border-bottom:1px solid #DDE1E5}'+
      '.specs-page table tr:last-child{border-bottom:none}'+
      '.specs-page table td{padding:10px 16px;font-size:12px;line-height:1.35;vertical-align:top;word-break:break-word}'+
      '.specs-page table td.lbl{color:#5F666E;font-weight:600;width:34%;background:#E8EBEE}'+
      '.specs-page table td.val{color:#252B31;font-weight:500;background:#F5F6F7}'+
      '.specs-page .specs-bottom-space{height:52px}'+
    '</style>'+
  '</div>';
}
...
async function generateSpecsPDF(){
  ...
  try {
    ...
    // La ficha está diseñada en proporción A4 (794x1123).
    // Se coloca completa en UNA sola hoja, respetando el margen inferior.
...
function renderSimilarTab(){
  try {
    ...
    similar.forEach(function(s){
      ...
      // Precio/descuento del plan y plazo actualmente seleccionados.
      // Usa resolvePrice, igual que Descuentos, para respetar las reglas reales del catálogo.
      const simPromo = (curPlan && typeof resolvePrice === 'function')
        ? resolvePrice(d.id, curPlan, curPlazo)
        : null;
      const simContado = Number(s.contado) || 0;
      const simTienePromo = simPromo !== null && simPromo !== undefined && simContado > 0;
      const simPromoNum = simTienePromo ? Number(simPromo) : null;
      const simPct = (simTienePromo && Number.isFinite(simPromoNum))
        ? Math.max(0, Math.round(((simContado - simPromoNum) / simContado) * 100))
        : 0;
      const simPrecioTxt = simTienePromo && Number.isFinite(simPromoNum)
        ? '$'+Math.round(simPromoNum).toLocaleString('es-MX')
        : '$'+Math.round(simContado).toLocaleString('es-MX');
      const simDescTxt = simTienePromo
        ? (simPromoNum === 0 ? 'Incluido' : '−'+simPct+'%')
        : 'contado';
      ...
      if(simTienePromo && Number.isFinite(simPromoNum) && simPromoNum !== simContado){
        h += '<div class="similar-contado"><s>$'+Math.round(simContado).toLocaleString('es-MX')+'</s> · '+d.storage+'</div>';
      }
      h += '</div>';
      h += '<div class="similar-price">'+simPrecioTxt+'<small>'+simDescTxt+'</small></div>';
      h += '</div>';
    });
...
const FIREBASE_CFG={
  apiKey: "AIzaSyBPw2YoD_oGtCrm0YCNIuLsgwy6N5OhqQk",
  authDomain: "cotizador-att.firebaseapp.com",
  projectId: "cotizador-att",
  storageBucket: "cotizador-att.firebasestorage.app",
  messagingSenderId: "188312063961",
  appId: "1:188312063961:web:7c6ce7d2c00ad54a5993c8",
  measurementId: "G-QM8DS6B6C0"
};
...
async function loadFirebase(){
  ...
  try{
    ...
    // Esperar a que Firebase restaure una sesión Auth persistida antes de
    // cualquier lectura protegida de Firestore.
...
async function doLogin(){
  ...
  if(!attuid){errEl.textContent='Ingresa tu usuario.';return;}
  ...
  try{
    ...
    // El usuario sigue escribiendo solo su Usuario/ATTUID. Firebase Auth usa
    // internamente un correo técnico que el colaborador no necesita conocer.
    const authEmail = attuid.toLowerCase() + '@cotizadorpro.app';
    try{
      await authFns.signInWithEmailAndPassword(firebaseAuth, authEmail, password);
    }catch(authErr){
      console.error('[Login/Auth] error:', authErr && authErr.code, authErr);
      ...
      if(code === 'auth/invalid-credential' ||
        ...
        errEl.textContent='Demasiados intentos. Espera un momento y vuelve a intentar.';
      }else if(code === 'auth/network-request-failed'){
        errEl.textContent='Error de conexión. Revisa tu internet y reintenta.';
      }else{
        errEl.textContent='No se pudo iniciar sesión. Reintenta.';
      }
      btn.disabled=false;btn.textContent='Entrar';
      return;
    }

    // Ya autenticado, las reglas permiten leer únicamente su propio perfil.
    const ref=firestoreFns.doc(firestoreDB,'empleados',attuid);
    const snap=await firestoreFns.getDoc(ref);
    if(!snap.exists()){
      try{ await authFns.signOut(firebaseAuth); }catch(_e){}
      errEl.textContent='Tu perfil no está configurado. Contacta a tu administrador.';
      btn.disabled=false;btn.textContent='Entrar';
      return;
    }
    const emp=snap.data();
    ...
    asesorData={
      ...
      tienda: emp.tienda||'',
      rol: (emp.rol||'asesor').toLowerCase(),
      tiendasAsignadas: emp.tiendasAsignadas || [],
      loginAt: Date.now()
    };
    saveSesion(asesorData);
    document.getElementById('asesor-overlay').classList.remove('show');
...
function doLogout(){
  ...
  try{
    if(firebaseAuth && authFns && authFns.signOut){
      authFns.signOut(firebaseAuth).catch(function(e){ console.warn('[Auth] signOut:', e); });
    }
  }catch(e){}
  clearSesion();
  asesorData=null;
...
if(lblEl){
  lblEl.textContent = 'cotizaste hoy';
    ...
    // Racha de actividad: primero cache local; después Firestore sincroniza
    // móvil y escritorio para el mismo ATTUID.
    function _pintarRacha(valor){
      if(!rachaEl) return;
      var txt = document.getElementById('hv2-racha-txt');
      if(valor >= 2){
        rachaEl.style.display = '';
        if(txt) txt.textContent = 'Racha de ' + valor + ' días cotizando';
      } else if(valor === 1){
        rachaEl.style.display = '';
    ...
    _pintarRacha((typeof calcularRacha==='function') ? calcularRacha() : 0);

    try{
      var pulsoReal=(typeof sincronizarPulsoEjecutivo==='function')
        ? await sincronizarPulsoEjecutivo() : null;
      if(pulsoReal){
        cotHoy=pulsoReal.hoy||0;
        if(numEl) numEl.textContent=cotHoy;
        _pintarRacha(pulsoReal.racha||0);
      }
    }catch(_e){}
  }
...
function openAsesorEdit(){
  ...
  attuidBox.innerHTML = '<div style="font-size:10px;color:var(--label3);text-transform:uppercase;letter-spacing:0.5px;margin-bottom:3px">ATTUID</div><div style="font-size:14px;font-weight:600;font-family:Roboto,sans-serif">'+asesorData.attuid+'</div>';
...
// [v1.12.2] SINCRONIZACIÓN ENTRE DISPOSITIVOS DEL EJECUTIVO.
// El contador y la racha nacieron en localStorage, por eso móvil y escritorio
// mostraban valores distintos. Firestore es ahora la fuente compartida y
// localStorage queda como cache instantáneo/offline.
async function sincronizarPulsoEjecutivo(){
  try{
    if(!asesorData || !asesorData.attuid) return null;
    await loadFirebase();
    if(!firestoreDB || !firestoreFns) return null;

    const attuid=String(asesorData.attuid);
    const q=firestoreFns.query(
      firestoreFns.collection(firestoreDB,'cotizaciones'),
      firestoreFns.where('attuid','==',attuid)
    );
    ...
    const dias=Object.keys(diasSet).sort().slice(-60);
    try{
      localStorage.setItem(_cotHoyKey(),JSON.stringify({fecha:hoy,n:hoyN}));
      localStorage.setItem(_diasActivosKey(),JSON.stringify(dias));
    }catch(_e){}

    return {hoy:hoyN,racha:calcularRacha(),dias:dias};
  }catch(e){
    console.warn('[Pulso] No se pudo sincronizar entre dispositivos:',e&&e.message);
    return null;
  }
}
...
async function contarEquipoHoy(){
  try{
    if(!asesorData || !asesorData.attuid) return null;
    var rol=_actividadRolOficial(asesorData.rol);
    var esMando=(rol==='gerente'||rol==='regional'||rol==='nacional'||rol==='director'||rol==='admin');
...
async function obtenerDashboard(periodo, region, tienda, tiendasAsignadas){
  ...
  const rolActual = _actividadRolOficial(asesorData && asesorData.rol);
  if(rolActual === 'regional' || rolActual === 'nacional' || rolActual === 'director' || rolActual === 'admin'){
    try{
      if(tienda){
      ...
      } else if(rolActual === 'nacional' || rolActual === 'director' || rolActual === 'admin'){
        ...
        } else if(rolActual === 'nacional' && asesorData && asesorData.region){
          asesoresTerritorio = await leerAsesoresDeTerritorio({region: asesorData.region});
        } else if(rolActual === 'director' || rolActual === 'admin'){
...
// [V46] Normalización de roles para Actividad.
// Jerarquía oficial: ADMIN1 > Director > Nacional > Regional > Gerente > Ejecutivo.
// Compatibilidad: asesor=ejecutivo, director_nacional=director.
function _actividadRolOficial(raw){
  var r=String(raw||'ejecutivo').trim().toLowerCase();
  if(r==='asesor') return 'ejecutivo';
  if(r==='director_nacional') return 'director';
  if(r==='admin' || (asesorData && String(asesorData.attuid||'').toUpperCase()==='ADMIN1')) return 'admin';
  return r;
}

function _actividadActualizarContexto(){
  try{
    var f=document.getElementById('dash-context-date');
    var g=document.getElementById('dash-context-greet');
    var n=document.getElementById('dash-context-count');
    var now=new Date();
    if(f) f.textContent=now.toLocaleDateString('es-MX',{weekday:'long',day:'numeric',month:'long'}).toUpperCase();
    if(g){ var nombre=(asesorData&&asesorData.name)||''; g.textContent=nombre?('Hola '+nombre.split(/\s+/)[0]):''; }
    if(n){
      var count=0;
      try{ count=(typeof contarCotizacionesHoy==='function')?(contarCotizacionesHoy()||0):0; }catch(e){}
...
function updateDashHomeCard(){
  ...
  card.style.display='flex';
  const rol=_actividadRolOficial(asesorData.rol);
  ...
  setT('Actividad');
  if(rol==='admin' || rol==='director' || rol==='nacional' || rol==='regional'){
    setS((rol==='admin'||rol==='director')?'Cotizaciones del país':(rol==='nacional'?'Cotizaciones de tus regiones':'Cotizaciones de tus tiendas'));
  } else if(rol==='gerente'){
...
function openDashboard(){
  if(!asesorData || !asesorData.attuid) return;
  // [V53] Actividad siempre se abre sobre Inicio como base. Así debajo del
  // menú solo existe el contenido de Actividad y no quedan restos de Catálogo/Accesos.
  try{
...
function abrirDashboardConSesionActual(){
  ...
  if(downloadBtn){
    const showDownload = (rol==='admin' || rol==='director' || rol==='nacional' || rol==='regional' || rol==='gerente' || rol==='ejecutivo');
  ...
  if(rol==='admin' || rol==='director'){
    // ADMIN1 y Director: visión nacional.
    title.textContent = rol==='admin' ? 'Actividad · ADMIN1' : 'Actividad · Director';
    regionRow.style.display='flex';
    poblarRegionSelect();
    regionSel.value='TODAS';
    regionSel.disabled=false;
  } else if(rol==='nacional'){
    // Nacional: una o varias regiones asignadas.
    const regs=(typeof _misRegiones==='function')?_misRegiones():(asesorData.region?[asesorData.region]:[]);
    title.textContent='Actividad · Nacional';
    regionRow.style.display='flex';
    poblarRegionSelect(regs);
    regionSel.disabled=false;
    if(regs.length){ regionSel.value=regs[0]; dashState.regionFiltro=regs[0]; }
  } else if(rol==='regional'){
    title.textContent='Actividad · Regional';
    ...
    title.textContent='Actividad · Gerente';
    regionRow.style.display='none';
    regionSel.disabled=true;
  ...
  _actividadActualizarContexto();

  // [V55] Actividad: ocultar explícitamente el resumen de Inicio.
  const wsSummary = document.querySelector('#s-home .ws-summary');
  if(wsSummary){
    wsSummary.dataset.v55PrevDisplay = wsSummary.style.display || '';
    wsSummary.style.setProperty('display','none','important');
  }

  ov.classList.add('show');
  document.body.classList.add('v55-actividad-abierta');
  document.body.style.overflow='hidden';
  reloadDashboard();
}
...
function closeDashboard(){
  ...
  document.body.classList.remove('v55-actividad-abierta');

  const wsSummary=document.querySelector('#s-home .ws-summary');
  if(wsSummary){
    wsSummary.style.removeProperty('display');
    if(wsSummary.dataset.v55PrevDisplay){
      wsSummary.style.display=wsSummary.dataset.v55PrevDisplay;
    }
    delete wsSummary.dataset.v55PrevDisplay;
  }
...
async function exportarDashboardExcel(){
  ...
  try {
    ...
    // ─── HOJA 1: RESUMEN ───────────────────────────────────────────────
    const ws1Data = [
      ['Cotizador Pro — Reporte de Actividad'],
      [],
    ...
    // [v77] ─── FUNNEL + DETALLE DE COTIZACIONES ─────────────────────────
    if(dashState.funnel && dashState.funnel.length){
      const funnelRows=[['Equipo','Plan','Plazo','Cotizaciones']];
      const combos={};
      dashState.funnel.forEach(function(c){
        const eq=c.equipoNombre||c.equipoId||'Sin equipo';
        const pl=c.plan||'Sin plan';
        const pz=c.plazo ? String(c.plazo)+' meses' : 'Sin plazo';
        const k=eq+'|||'+pl+'|||'+pz;
        if(!combos[k]) combos[k]=[eq,pl,pz,0];
        combos[k][3]++;
      });
      Object.keys(combos).map(function(k){return combos[k];})
        .sort(function(a,b){return b[3]-a[3];})
        .forEach(function(r){funnelRows.push(r);});
      const wsFun=XLSX.utils.aoa_to_sheet(funnelRows);
      wsFun['!cols']=[{wch:34},{wch:18},{wch:14},{wch:14}];
      XLSX.utils.book_append_sheet(wb,wsFun,'Equipos Planes Plazos');

      const det=[['Fecha','Hora','ATTUID','Ejecutivo','Tienda','Región','Equipo','Marca','Plan','Plazo','Renta plan','Precio equipo','Portabilidad','Seguro','Control','Tipo operación']];
      dashState.funnel.forEach(function(c){
        let hora='';
        try{ hora=c.timestamp ? new Date(c.timestamp).toLocaleTimeString('es-MX',{hour:'2-digit',minute:'2-digit',hour12:false}) : ''; }catch(e){}
        det.push([
          c.fecha||'',hora,c.attuid||'',c.nombre||'',c.tienda||'',c.region||'',
          c.equipoNombre||c.equipoId||'',c.equipoMarca||'',c.plan||'',c.plazo||'',
          c.planRenta||0,c.precioEquipo||0,c.conPortabilidad?'Sí':'No',
          c.conSeguro?'Sí':'No',c.conControl?'Sí':'No',c.tipoOperacion||''
        ]);
      });
      const wsDet=XLSX.utils.aoa_to_sheet(det);
      wsDet['!cols']=[{wch:12},{wch:8},{wch:12},{wch:28},{wch:24},{wch:16},{wch:34},{wch:14},{wch:16},{wch:10},{wch:14},{wch:16},{wch:13},{wch:10},{wch:10},{wch:16}];
      XLSX.utils.book_append_sheet(wb,wsDet,'Detalle cotizaciones');
    }

    // ─── GENERAR Y DESCARGAR ──────────────────────────────────────────
    const filename = 'CotizadorPro_Actividad_' + hoy + '_' + (data.periodo||'') + '.xlsx';
...
// [v1.12.1] Respaldo de Mi actividad con documentos reales de /cotizaciones.
// Evita que una cotización existente desaparezca de Hoy/Ayer si /resumenes quedó incompleto.
async function leerCotizacionesRealesAsesor(dias){
  try{
    if(!asesorData || !asesorData.attuid || !Array.isArray(dias) || !dias.length) return null;
    await loadFirebase();
    const attuid=String(asesorData.attuid);
    const wanted=new Set(dias);
    const q=firestoreFns.query(
      firestoreFns.collection(firestoreDB,'cotizaciones'),
      firestoreFns.where('attuid','==',attuid)
    );
    ...
    return {total:total,porDia:porDia};
  }catch(e){
    console.warn('[Actividad] No se pudieron comprobar cotizaciones reales:',e && e.message);
    return null;
  }
}
...
async function reloadDashboard(){
  ...
  invalidarDashCache();
  try{
    // [v78] Antes de leer Actividad, intentar subir cotizaciones que sigan
    // en la cola local. El contador del Home es inmediato/local, mientras
    // Actividad usa Firestore; esto evita que ambos se vean desfasados.
    if(rol==='ejecutivo' && typeof flushColaCotizaciones==='function'){
      try{ await flushColaCotizaciones(); }catch(_syncErr){}
    }
    ...
    if(rol === 'admin' || rol === 'director' || rol === 'nacional'){
      promesas.push(leerRegionales());
    }
    const results = await Promise.all(promesas);
    const data = results[0];
    // Asesor: contrastar el agregado diario con las cotizaciones reales.
    // Los roles gerenciales siguen usando /resumenes para no disparar lecturas masivas.
    if(rol === 'ejecutivo'){
      data.miActividadReal = await leerCotizacionesRealesAsesor(data.rango.dias);
    }
    if(results.length > 1){
      // Filtrar regionales por la región del director (o todos si es nacional sin filtro)
      const dirRegion = (rol === 'nacional')
    ...
    if(rol==='admin' || rol==='director' || rol==='nacional' || rol==='regional' || rol==='gerente'){
      renderDashboardManager(data);
    } else {
      renderMiActividad(data);
    }
    // [v77] Funnel disponible para Ejecutivo y toda la jerarquía superior.
    await montarFunnelActividad(data);
  }catch(e){
...
function renderMiActividad(data){
  ...
  const _miResumen=(data.rango.dias.length>0)
    ? ((_cacheC.asesor||{})[_k(yo)]||0)
    : 0;
  const _miReal=(data.miActividadReal && typeof data.miActividadReal.total==='number')
    ? data.miActividadReal.total
    : null;
  // Si el resumen diario quedó incompleto, no ocultar actividad que sí existe.
  // Math.max conserva compatibilidad con datos históricos ya agregados.
  let miCount=(_miReal===null) ? _miResumen : Math.max(_miResumen,_miReal);
  // [v78] El Home cuenta la cotización en el instante en que se genera y la
  // encola localmente. Si Firestore aún no terminó de sincronizar, Actividad
  // conserva ese mismo total para Hoy/semana/mes cuando el rango incluye hoy.
  try{
    if(data.rango.dias.indexOf(_hoyISO())>=0){
      miCount=Math.max(miCount, contarCotizacionesHoy()||0);
    }
  }catch(_e){}
...
const ADM_ROLES_MANDO = ['regional', 'nacional', 'director', 'director_nacional'];

// [V45] Roles oficiales: Ejecutivo → Gerente → Regional → Nacional → Director → ADMIN1.
// Alias heredados: asesor = ejecutivo; director_nacional = director.
function _admRol(d){
  const r=String((d && d.rol) || 'ejecutivo').trim().toLowerCase();
  if(r==='asesor') return 'ejecutivo';
  if(r==='director_nacional') return 'director';
  return r;
}
...
function _meRol(){ const r=String((asesorData && asesorData.rol) || '').trim().toLowerCase(); if(r==='asesor') return 'ejecutivo'; if(r==='director_nacional') return 'director'; return r; }
...
function _meEsGlobal(){ return _meEsSuper() || _meRol() === 'director'; }
...
function _admHeroSub(){
  ...
  const r = _meRol();
  if(r==='director') return 'Administra accesos a nivel nacional';
  if(r==='nacional') return 'Administra el acceso de tus regiones';
  if(r==='regional') return 'Administra el acceso de tu equipo';
  if(r==='gerente') return 'Administra el acceso de tu tienda';
...
function showAccesos(){
  ...
  // [V45] La identidad técnica sigue siendo `attuid`, pero la UI habla de Usuario.
  if(i) i.placeholder = 'Ej. NACIONAL_A';
  try{
    const sec=i && i.parentElement ? i.parentElement.parentElement : null;
    if(sec){
      const labs=sec.querySelectorAll('*');
      labs.forEach(function(el){ if(el.childElementCount===0 && /BUSCAR POR ATTUID/i.test(el.textContent||'')) el.textContent='BUSCAR POR USUARIO'; });
    }
  }catch(e){}
  adminCargarEquipo();
}
...
function _admEnAlcance(d){
  ...
  if(_meRol()==='nacional') return _misRegiones().indexOf(String(d.region||'')) >= 0;
  if(_meRol()==='regional') return _meTiendas().indexOf(String(d.tienda||'')) >= 0;
  if(_meRol()==='gerente') return String(d.tienda||'') === String((asesorData&&asesorData.tienda)||'');
  return false;
}
...
function _admRoleTxt(d){
  const r = _admRol(d);
  return ({ejecutivo:'Ejecutivo',gerente:'Gerente',regional:'Regional',nacional:'Nacional',director:'Director',admin:'ADMIN1'})[r] || r;
}
...
// ── Buscar por Usuario (internamente ATTUID) ──
async function adminBuscar(){
  ...
  try{
    ...
    const ref = firestoreFns.doc(firestoreDB,'empleados',attuid);
    const snap = await firestoreFns.getDoc(ref);
    if(!snap.exists()){ out.innerHTML='<div class="adm-msg">No existe un colaborador con usuario <b>'+_admEsc(attuid)+'</b>.</div>'; return; }
    const d = snap.data()||{};
    _admGente[attuid] = d;
...
function _admConstruirArbol(docs){
  // [V45] Árbol oficial. ADMIN1 es raíz lógica y NO se cuenta como región/tienda/persona.
  const activos=docs.filter(function(x){ return x.id!==SUPER_ADMIN_ATTUID && x.d && x.d.activo!==false; });
  const directores=activos.filter(function(x){ return _admRol(x.d)==='director'; });
  const nacionales=activos.filter(function(x){ return _admRol(x.d)==='nacional'; });
  const regionales=activos.filter(function(x){ return _admRol(x.d)==='regional'; });
  const operativos=activos.filter(function(x){ const r=_admRol(x.d); return r==='gerente'||r==='ejecutivo'; });
  const regiones={};
  function addRegion(r){ r=String(r||'').trim(); if(r && r.toLowerCase()!=='global') regiones[r]=true; }
  nacionales.forEach(function(x){ (Array.isArray(x.d.regionesAsignadas)?x.d.regionesAsignadas:[x.d.region]).forEach(addRegion); });
  regionales.forEach(function(x){ addRegion(x.d.region); });
  operativos.forEach(function(x){ addRegion(x.d.region); });
  const arr=Object.keys(regiones).sort().map(function(region){
    const nac=nacionales.filter(function(x){ const a=Array.isArray(x.d.regionesAsignadas)?x.d.regionesAsignadas:[]; return String(x.d.region||'')===region || a.map(String).indexOf(region)>=0; });
    const regs=regionales.filter(function(x){ return String(x.d.region||'')===region; });
    const tiendas={};
    operativos.filter(function(x){ return String(x.d.region||'')===region; }).forEach(function(x){ const t=String(x.d.tienda||'').trim(); if(t) (tiendas[t]=tiendas[t]||[]).push(x); });
    regs.forEach(function(x){ (Array.isArray(x.d.tiendasAsignadas)?x.d.tiendasAsignadas:[]).forEach(function(t){ t=String(t||'').trim(); if(t) tiendas[t]=tiendas[t]||[]; }); });
    const ts=Object.keys(tiendas).sort().map(function(t){ return {nombre:t,gente:tiendas[t],personas:tiendas[t].length}; });
    return {region:region,nacionales:nac,regionales:regs,tiendas:ts,personas:operativos.filter(function(x){return String(x.d.region||'')===region;}).length,nTiendas:ts.length};
  });
  return {directores:directores, regiones:arr};
}
function _admRenderArbol(){
  let h='';
  if(_admArbol.directores.length){
    h+='<div class="adm-group open"><div class="adm-group-head"><span class="adm-group-name">DIRECTOR</span><span class="adm-group-count">'+_admArbol.directores.length+'</span></div><div class="adm-group-body">';
    _admArbol.directores.forEach(function(x){ h+=_admTarjeta(x.id,x.d); }); h+='</div></div>';
  }
  _admArbol.regiones.forEach(function(R,i){
    h+='<div class="adm-group adm-grp-region" id="adm-v45-r'+i+'"><div class="adm-group-head" onclick="admV45ToggleRegion('+i+')">'+_admChev()+'<span class="adm-group-name">REGIÓN '+_admEsc(R.region)+'</span><span class="adm-group-count">'+R.nTiendas+' tiendas · '+R.personas+'</span></div><div class="adm-group-body" id="adm-v45-rb'+i+'"></div></div>';
  });
  return h || '<div class="adm-msg">Aún no hay Director, Nacional, Regional, Gerente o Ejecutivo registrados.</div>';
}
function admV45ToggleRegion(i){
  const grp=document.getElementById('adm-v45-r'+i), body=document.getElementById('adm-v45-rb'+i), R=_admArbol.regiones[i];
  if(body && !body.getAttribute('data-l')){
    let h='<div class="adm-nest">';
    if(R.nacionales.length){ h+='<div class="adm-group open"><div class="adm-group-head"><span class="adm-group-name">NACIONAL</span><span class="adm-group-count">'+R.nacionales.length+'</span></div><div class="adm-group-body">'; R.nacionales.forEach(function(x){h+=_admTarjeta(x.id,x.d);}); h+='</div></div>'; }
    if(R.regionales.length){ h+='<div class="adm-group open"><div class="adm-group-head"><span class="adm-group-name">REGIONALES</span><span class="adm-group-count">'+R.regionales.length+'</span></div><div class="adm-group-body">'; R.regionales.forEach(function(x){h+=_admTarjeta(x.id,x.d);}); h+='</div></div>'; }
    R.tiendas.forEach(function(T,k){ h+='<div class="adm-group" id="adm-v45-r'+i+'t'+k+'"><div class="adm-group-head" onclick="admV45ToggleTienda('+i+','+k+')">'+_admChev()+'<span class="adm-group-name">'+_admEsc(T.nombre)+'</span><span class="adm-group-count">'+T.personas+' personas</span></div><div class="adm-group-body" id="adm-v45-rb'+i+'t'+k+'"></div></div>'; });
    h+='</div>'; body.innerHTML=h; body.setAttribute('data-l','1');
  }
  if(grp) grp.classList.toggle('open');
}
function admV45ToggleTienda(i,k){
  const grp=document.getElementById('adm-v45-r'+i+'t'+k), body=document.getElementById('adm-v45-rb'+i+'t'+k), T=_admArbol.regiones[i].tiendas[k];
  if(body && !body.getAttribute('data-l')){
    const ger=T.gente.filter(function(x){return _admRol(x.d)==='gerente';});
    const eje=T.gente.filter(function(x){return _admRol(x.d)==='ejecutivo';});
    let h='<div class="adm-nest">';
    if(ger.length){ h+='<div class="adm-group open"><div class="adm-group-head"><span class="adm-group-name">GERENTE</span><span class="adm-group-count">'+ger.length+'</span></div><div class="adm-group-body">'; ger.forEach(function(x){h+=_admTarjeta(x.id,x.d);}); h+='</div></div>'; }
    if(eje.length){ h+='<div class="adm-group open"><div class="adm-group-head"><span class="adm-group-name">EJECUTIVOS</span><span class="adm-group-count">'+eje.length+'</span></div><div class="adm-group-body">'; eje.forEach(function(x){h+=_admTarjeta(x.id,x.d);}); h+='</div></div>'; }
    if(!ger.length&&!eje.length) h+='<div class="adm-msg">Sin personal asignado.</div>';
    h+='</div>'; body.innerHTML=h; body.setAttribute('data-l','1');
  }
  if(grp) grp.classList.toggle('open');
}
...
async function adminCargarEquipo(){
  ...
  try{
    ...
    const _usarEquipo = _meTiendas().length>0 || (_meRol()==='nacional' && (_meRegion() || _misRegiones().length>0));
    if(_meEsGlobal() && !_usarEquipo){
      ...
      let totPers = 0; _admArbol.regiones.forEach(function(R){ totPers += R.personas; });
      const totTiendas = _admArbol.regiones.reduce(function(a,R){ return a + R.nTiendas; }, 0);
      if(scopeRegion){ scopeRegion.textContent = 'Cobertura nacional'; scopeRegion.style.display='block'; }
      if(scopeBox){
        scopeBox.innerHTML = _admScopeCell(_admArbol.regiones.length,'Regiones',false)
      ...
      groups.innerHTML = _admRenderArbol();
      return;
    }
    ...
    let h='', gi=0;
    if(_meRol()==='nacional' || _meEsGlobal()){
...
// [v1.11.64] Flyer comparativo de DOS EQUIPOS completos.
// Cada bloque conserva su propio equipo, plan, plazo, enganche, servicios y portabilidad.
function buildFlyerTwoDevicesHTML(a,b){
  function calc(s){
    var eng=s.engPct!==null?Math.round(s.promo*s.engPct/100):(s.engCustom||0);
    var rentas=(s.rentas||0)*s.planRenta;
    var deposito=s.deposito||0;
    var inicial=eng+rentas+deposito;
    var seguro=s.seguro?getSeguroPrice(s.contado):0;
    var control=s.control?50:0;
    var rem=Math.max(0,s.promo-eng);
    var eq=Math.round(rem/s.plazo);
    var mensual=s.planRenta+eq+seguro+control;
    var pct=s.plan==='Titanio'?0.10:0.20;
    var mensualPort=Math.round(s.planRenta*(1-pct))+eq+seguro+control;
    var desc=s.contado>0?Math.round((1-s.promo/s.contado)*100):0;
    var ahorro=Math.max(0,(s.contado||0)-(s.promo||0));
    return {inicial:inicial,eq:eq,mensual:mensual,mensualPort:mensualPort,
            seguro:seguro,control:control,desc:desc,ahorro:ahorro};
  }

  function option(s,n){
    var c=calc(s);
    var accent=FLYER_PLAN_ACCENTS[s.plan]||'#00A8E0';
    var h='<section class="flyer-duo-option">';
    h+='<div class="flyer-duo-option-head"><span>OPCIÓN '+n+'</span><b>'+s.plan+' · '+s.plazo+' meses</b></div>';

    h+='<div class="flyer-duo-hero">';
    h+=_flyerProducto(s);
    h+='<div class="flyer-duo-price">';
    h+='<div class="flyer-v3-price-lbl">Precio del equipo</div>';
    if(s.contado>s.promo) h+='<div class="flyer-v3-strike">$'+_flyerFmx(Math.round(s.contado))+'</div>';
    if(s.promo===0){
      h+='<div class="flyer-duo-price-big included">Incluido*</div>';
    }else{
      h+='<div class="flyer-duo-price-big">$'+_flyerFmx(Math.round(s.promo))+'</div>';
      if(c.ahorro>0) h+='<div class="flyer-v3-save">Ahorras $'+_flyerFmx(c.ahorro)+'</div>';
    }
    if(c.desc>=10 && s.promo>0){
      h+='<div class="flyer-duo-discount" style="background:'+accent+'">−'+c.desc+'% <small>descuento</small></div>';
    }
    h+='</div></div>';

    h+='<div class="flyer-duo-plan" style="border-left-color:'+accent+'">';
    h+='<div><span>Plan</span><b>'+s.plan+'</b><small>'+s.plazo+' meses</small></div>';
    h+='<div><span>Renta</span><b>$'+_flyerFmx(s.planRenta)+'</b><small>/mes</small></div>';
    h+='</div>';

    h+='<div class="flyer-duo-payments">';
    h+='<div><span>Pago inicial</span><b>$'+_flyerFmx(c.inicial)+'</b></div>';
    h+='<div class="flyer-duo-month"><span>Mensualidad total</span><b>$'+_flyerFmx(c.mensual)+'<small>/mes</small></b></div>';
    h+='</div>';

    var det=['$'+_flyerFmx(s.planRenta)+' plan'];
    if(c.eq>0) det.push('$'+_flyerFmx(c.eq)+' equipo');
    if(c.control>0) det.push('$'+_flyerFmx(c.control)+' control');
    if(c.seguro>0) det.push('$'+_flyerFmx(c.seguro)+' seguro');
    h+='<div class="flyer-duo-detail">'+det.join(' + ')+'</div>';

    if(s.port){
      h+='<div class="flyer-duo-port" style="border-left-color:'+accent+'"><b>Portabilidad '+(s.plan==='Titanio'?'10':'20')+'%</b>';
      h+=' · $'+_flyerFmx(c.mensualPort)+'/mes · facturas 2 a 7</div>';
    }
    h+='</section>';
    return h;
  }
  ...
  var greet=b||a;
  if(!greet.cliente && a.cliente) greet.cliente=a.cliente;

  var h='<div class="flyer-v3 flyer-v3-device-compare flyer-v3-device-compare-v31">';
  h+=_flyerHead();
  h+=_flyerGreet(greet);
  h+='<div class="flyer-duo-title"><b>Compara tus equipos</b><span>Dos opciones, una misma cotización</span></div>';
  h+=option(a,1);
  h+=option(b,2);
  if(a.promo===0 || b.promo===0){
    h+='<div class="flyer-cmp-note">* Equipo sin costo sujeto a permanencia. Cancelación anticipada genera cobro del equipo.</div>';
  }
  var _cmpFooter=_flyerFooter();
  _cmpFooter=_cmpFooter.replace(
    '* Pago inicial sujeto a aprobación crediticia · Consulta términos y condiciones · Precios sujetos a cambio sin previo aviso',
    '* Pago inicial sujeto a aprobación crediticia · Consulta términos y condiciones · Precios sujetos a cambio sin previo aviso.'
  );
  h+=_cmpFooter;
  h+='</div>';
...
async function generateFlyerImage(){
  ...
  try{
    ...
    renderEl.innerHTML = (cotEquipoComparar && cotEquipoComparar.device)
      ? buildFlyerTwoDevicesHTML(cotEquipoComparar, cotState)
      : ((cotState.comparar && cotState.comparar.length)
...
// [v1.11.66] Construye una cotización base desde la selección visible.
// Esto permite comparar equipos SIN abrir "Personaliza tu cotización".
function cotStateDesdeSeleccionPrincipal(){
  // Fuente principal: estado real del cotizador.
  var fichaId=curFichaId;
  var plan=curPlan;
  var plazo=curPlazo;

  // Respaldo visual: si por alguna reconstrucción del panel una variable quedó
  // desincronizada, leer exactamente lo que el asesor ve seleccionado.
  if(!plan){
    var pAct=document.querySelector('.plan-chip.active');
    if(pAct) plan=pAct.dataset.plan || (pAct.textContent||'').trim();
  }
  if(!plazo){
    var zAct=document.querySelector('.plazo-btn.active');
    if(zAct){
      var zm=(zAct.textContent||'').match(/\d+/);
      if(zm) plazo=Number(zm[0]);
    }
  }

  var all=[].concat(CAT.ios||[],CAT.android||[]);
  var dev=fichaId ? all.find(function(d){return d.id===fichaId;}) : null;

  // Último respaldo: resolver el equipo por el nombre que aparece en la ficha.
  if(!dev){
    var title=document.getElementById('f-title');
    var nm=title ? (title.textContent||'').trim() : '';
    if(nm) dev=all.find(function(d){return d.name===nm;});
    if(dev) fichaId=dev.id;
  }

  if(!dev || !plan || !plazo) return null;

  var info=PRICES[fichaId];
  if(!info) return null;

  var promo=resolvePrice(fichaId,plan,plazo);
  if(promo===null || promo===undefined) return null;

  var planObj=(typeof PLANS_DATA!=='undefined' && PLANS_DATA)
    ? PLANS_DATA.find(function(p){return p.name===plan;})
    : null;
  var renta=planObj && planObj.renta!=null ? Number(planObj.renta) : 0;
  if(!renta){
    var RENTAS={'Azul 1':330,'Azul 2':435,'Azul 3':550,'Plata':650,'Oro':725,'Black':825,'Platino':1035,'Diamante':1300,'Titanio':1599};
    renta=RENTAS[plan]||0;
  }

  var desc=info.contado ? Math.round((1-promo/info.contado)*100) : 0;
  var bid=fichaId;
  for(var k in (window.STORAGE_VARIANTS||{})){
    if(((window.STORAGE_VARIANTS[k]||[]).some(function(v){return v[1]===fichaId;}))){
      bid=k; break;
    }
  }
  var fp=(window.FUTURE_PRICES||{})[bid] || (window.FUTURE_PRICES||{})[fichaId];
  var upcoming=Array.isArray(window.UPCOMING_ONLY) &&
    (window.UPCOMING_ONLY.indexOf(bid)>=0 || window.UPCOMING_ONLY.indexOf(fichaId)>=0);

  return {
    port:false,control:false,seguro:false,
    engPct:0,engCustom:null,rentas:0,deposito:0,
    device:dev,plan:plan,plazo:Number(plazo),promo:promo,contado:info.contado,
    isHighDisc:desc>=70,descPct:desc,planRenta:renta,
    isUpcoming:upcoming,futStart:fp?fp.start:null,
    cliente:'',tipo:null,comparar:[]
  };
}

console.log('✅ Cotizador Pro v24 cargado — comparativa directa de equipos');

console.log('✅ Cotizador Pro v25 cargado — lectura robusta de equipo/plan/plazo');

console.log('✅ Cotizador Pro v27 — comparativa nativa cargada');

console.log('✅ Cotizador Pro v28 — build 20260921-v28-comparativa-final');

console.log('✅ Cotizador Pro v29 — comparativa limpia / estado único');



// [v33] Configurador completo compartido por escritorio y móvil.
let _cmpCfgState=null,_cmpCfgDone=null,_cmpCfgPhase=1;
function _cmpCfgMoney(n){return '$'+Math.round(Number(n)||0).toLocaleString('es-MX');}
function _cmpCfgCalc(){const s=_cmpCfgState;if(!s)return{inicial:0,mensual:0};const eng=s.engCustom!==null?Number(s.engCustom||0):Math.round((s.promo||0)*Number(s.engPct||0)/100);const rentas=Number(s.rentas||0)*Number(s.planRenta||0);const dep=Number(s.deposito||0);const eq=Math.round(Math.max(0,(s.promo||0)-eng)/Number(s.plazo||1));const seguro=s.seguro?getSeguroPrice(s.contado):0;const control=s.control?50:0;const plan=Number(s.planRenta||0);const pe=s.port?Math.round(plan*(s.plan==='Titanio'?0.90:0.80)):plan;return{inicial:eng+rentas+dep,mensual:pe+eq+seguro+control};}
function _cmpCfgEnsure(){let o=document.getElementById('cp-cmpcfg-overlay');if(o)return o;o=document.createElement('div');o.id='cp-cmpcfg-overlay';o.className='cp-cmpcfg-overlay';o.innerHTML='<div class="cp-cmpcfg-card"><div class="cp-cmpcfg-head"><div><div class="cp-cmpcfg-kicker" id="cp-cmpcfg-kicker"></div><div class="cp-cmpcfg-title" id="cp-cmpcfg-title"></div><div class="cp-cmpcfg-sub" id="cp-cmpcfg-sub"></div></div><button class="cp-cmpcfg-x" type="button" onclick="closeDeviceCompareConfigurator()">×</button></div><div class="cp-cmpcfg-body"><div class="cp-cmpcfg-sec" id="cp-cmpcfg-client-sec"><div class="cp-cmpcfg-label">Nombre del cliente</div><input id="cp-cmpcfg-client" class="cp-cmpcfg-client-input" type="text" autocomplete="name" placeholder="Ejemplo: Francisco"><div class="cp-cmpcfg-client-note">Aparecerá en el encabezado de la comparativa.</div></div><div class="cp-cmpcfg-sec"><div class="cp-cmpcfg-label">Pago inicial · enganche</div><div class="cp-cmpcfg-pills" id="cp-cmpcfg-eng"></div><div class="cp-cmpcfg-custom" id="cp-cmpcfg-engcustom"><input id="cp-cmpcfg-enginput" type="number" min="0" placeholder="Monto de enganche"></div></div><div class="cp-cmpcfg-sec"><div class="cp-cmpcfg-label">Mensualidades adelantadas / rentas en garantía</div><div class="cp-cmpcfg-pills" id="cp-cmpcfg-rentas"></div></div><div class="cp-cmpcfg-sec"><div class="cp-cmpcfg-label">Garantía</div><div class="cp-cmpcfg-pills"><button class="cp-cmpcfg-pill" id="cp-cmpcfg-dep0" type="button">Sin depósito</button><button class="cp-cmpcfg-pill" id="cp-cmpcfg-depx" type="button">$ Otro monto</button></div><div class="cp-cmpcfg-custom" id="cp-cmpcfg-depcustom"><input id="cp-cmpcfg-depinput" type="number" min="0" placeholder="Monto del depósito"></div></div><div class="cp-cmpcfg-sec"><div class="cp-cmpcfg-label">Beneficios y servicios</div><div class="cp-cmpcfg-switch" id="cp-cmpcfg-port"><div><b>Descuento por portabilidad</b><small>20% en el plan · Titanio 10% · facturas 2 a 7</small></div><div class="cp-cmpcfg-toggle"></div></div><div class="cp-cmpcfg-switch" id="cp-cmpcfg-control"><div><b>Control de datos</b><small>+$50 al mes</small></div><div class="cp-cmpcfg-toggle"></div></div><div class="cp-cmpcfg-switch" id="cp-cmpcfg-seguro"><div><b>Seguro AT&T Protección</b><small id="cp-cmpcfg-segurodesc"></small></div><div class="cp-cmpcfg-toggle"></div></div></div><div class="cp-cmpcfg-summary"><div><span>Pago inicial</span><b id="cp-cmpcfg-inicial">$0</b></div><div><span>Mensualidad estimada</span><b id="cp-cmpcfg-mensual">$0</b></div></div></div><div class="cp-cmpcfg-foot"><button class="cp-cmpcfg-btn cp-cmpcfg-cancel" type="button" onclick="closeDeviceCompareConfigurator()">Cancelar</button><button class="cp-cmpcfg-btn cp-cmpcfg-save" id="cp-cmpcfg-save" type="button">Guardar opción</button></div></div>';document.body.appendChild(o);o.addEventListener('click',function(e){if(e.target===o)closeDeviceCompareConfigurator();});return o;}
function _cmpCfgSummary(){const c=_cmpCfgCalc(),a=document.getElementById('cp-cmpcfg-inicial'),m=document.getElementById('cp-cmpcfg-mensual');if(a)a.textContent=_cmpCfgMoney(c.inicial);if(m)m.textContent=_cmpCfgMoney(c.mensual)+'/mes';}
function _cmpCfgRender(){const s=_cmpCfgState;if(!s)return;_cmpCfgEnsure();document.getElementById('cp-cmpcfg-kicker').textContent='Configurar Opción '+_cmpCfgPhase;document.getElementById('cp-cmpcfg-title').textContent=s.device.name;document.getElementById('cp-cmpcfg-sub').textContent=s.plan+' · '+s.plazo+' meses · renta '+_cmpCfgMoney(s.planRenta)+'/mes';
const csec=document.getElementById('cp-cmpcfg-client-sec'),cin=document.getElementById('cp-cmpcfg-client');
if(csec)csec.style.display=_cmpCfgPhase===1?'block':'none';
if(cin){if(document.activeElement!==cin)cin.value=s.cliente||'';cin.oninput=function(){s.cliente=this.value;};}
const eg=document.getElementById('cp-cmpcfg-eng');eg.innerHTML=[0,10,20,30,40,50].map(p=>'<button type="button" class="cp-cmpcfg-pill '+(s.engCustom===null&&Number(s.engPct)===p?'on':'')+'" data-e="'+p+'">'+p+'%</button>').join('')+'<button type="button" class="cp-cmpcfg-pill '+(s.engCustom!==null?'on':'')+'" data-e="x">$ Otro monto</button>';eg.querySelectorAll('[data-e]').forEach(b=>b.onclick=function(){const v=this.dataset.e;if(v==='x'){s.engPct=null;s.engCustom=Number(document.getElementById('cp-cmpcfg-enginput').value||0);}else{s.engPct=Number(v);s.engCustom=null;}_cmpCfgRender();if(v==='x')document.getElementById('cp-cmpcfg-enginput').focus();});
const ew=document.getElementById('cp-cmpcfg-engcustom'),ei=document.getElementById('cp-cmpcfg-enginput');ew.classList.toggle('show',s.engCustom!==null);if(s.engCustom!==null&&document.activeElement!==ei)ei.value=s.engCustom||'';ei.oninput=function(){s.engCustom=Number(this.value||0);_cmpCfgSummary();};
const rr=document.getElementById('cp-cmpcfg-rentas');rr.innerHTML=[0,1,2,3].map(n=>'<button type="button" class="cp-cmpcfg-pill '+(Number(s.rentas)===n?'on':'')+'" data-r="'+n+'">'+(n===0?'0':n+' renta'+(n>1?'s':''))+'</button>').join('');rr.querySelectorAll('[data-r]').forEach(b=>b.onclick=function(){s.rentas=Number(this.dataset.r);_cmpCfgRender();});
const dep=Number(s.deposito||0),d0=document.getElementById('cp-cmpcfg-dep0'),dx=document.getElementById('cp-cmpcfg-depx'),dw=document.getElementById('cp-cmpcfg-depcustom'),di=document.getElementById('cp-cmpcfg-depinput');d0.classList.toggle('on',!dep);dx.classList.toggle('on',dep>0);dw.classList.toggle('show',dep>0);d0.onclick=function(){s.deposito=0;_cmpCfgRender();};dx.onclick=function(){if(!s.deposito)s.deposito=.01;_cmpCfgRender();setTimeout(()=>document.getElementById('cp-cmpcfg-depinput').focus(),0);};if(dep>0&&document.activeElement!==di)di.value=dep===.01?'':dep;di.oninput=function(){s.deposito=Number(this.value||0);_cmpCfgSummary();};
[['port','cp-cmpcfg-port'],['control','cp-cmpcfg-control'],['seguro','cp-cmpcfg-seguro']].forEach(x=>{const el=document.getElementById(x[1]);el.classList.toggle('on',!!s[x[0]]);el.onclick=function(){s[x[0]]=!s[x[0]];_cmpCfgRender();};});document.getElementById('cp-cmpcfg-segurodesc').textContent='+'+_cmpCfgMoney(getSeguroPrice(s.contado))+' al mes';document.getElementById('cp-cmpcfg-save').textContent=_cmpCfgPhase===1?'Guardar Opción 1':'Generar comparativa';document.getElementById('cp-cmpcfg-save').onclick=function(){const cb=_cmpCfgDone,out=JSON.parse(JSON.stringify(_cmpCfgState));closeDeviceCompareConfigurator();if(cb)cb(out);};_cmpCfgSummary();}
function openDeviceCompareConfigurator(base,phase,done){_cmpCfgState=JSON.parse(JSON.stringify(base));
if((phase||1)===2&&cotEquipoComparar&&cotEquipoComparar.cliente)_cmpCfgState.cliente=cotEquipoComparar.cliente;_cmpCfgState.engPct=Number(_cmpCfgState.engPct||0);_cmpCfgState.engCustom=null;_cmpCfgState.rentas=Number(_cmpCfgState.rentas||0);_cmpCfgState.deposito=Number(_cmpCfgState.deposito||0);_cmpCfgPhase=phase||1;_cmpCfgDone=done||null;_cmpCfgEnsure().classList.add('show');_cmpCfgRender();}
function closeDeviceCompareConfigurator(){const o=document.getElementById('cp-cmpcfg-overlay');if(o)o.classList.remove('show');_cmpCfgDone=null;}
window.openDeviceCompareConfigurator=openDeviceCompareConfigurator;window.closeDeviceCompareConfigurator=closeDeviceCompareConfigurator;window.setDeviceCompareOption1=s=>{cotEquipoComparar=s?JSON.parse(JSON.stringify(s)):null;};window.getDeviceCompareOption1=()=>cotEquipoComparar?JSON.parse(JSON.stringify(cotEquipoComparar)):null;window.clearDeviceCompareOption1=()=>{cotEquipoComparar=null;};

// [v30] API mínima para el workspace de escritorio.
// Reutiliza el mismo motor de estado del generador normal.
window.captureQuoteState=function(deviceId,plan,plazo){
  // [v31] Captura silenciosa: NO llama showFicha(), NO cambia de pantalla
  // y NO deja visible el generador antiguo detrás de la vista previa.
  const prevId=curFichaId, prevPlan=curPlan, prevPlazo=curPlazo;
  try{
    curFichaId=deviceId;
    curPlan=plan;
    curPlazo=Number(plazo);
    const st=_crearCotStateActual();
    if(!st) return null;
...
window.generateDeviceComparison=function(opcion1,opcion2){
  if(!opcion1||!opcion2){alert('Falta configurar una de las opciones.');return;}
  if(opcion1.cliente&&!opcion2.cliente)opcion2.cliente=opcion1.cliente;
  if(opcion2.cliente&&!opcion1.cliente)opcion1.cliente=opcion2.cliente;
  cotEquipoComparar=JSON.parse(JSON.stringify(opcion1));
  cotState=JSON.parse(JSON.stringify(opcion2));
  return generateFlyerImage();
};

console.log('✅ Cotizador Pro v30 — comparación conectada al workspace desktop real');

console.log('✅ Cotizador Pro v33 — comparativa completa responsive desktop/mobile');

console.log('✅ Cotizador Pro v34 — fix cotización simple dev scope');

console.log('✅ Cotizador Pro v35 — comparativa estilo simple + cliente + AT&T');

console.log('✅ Cotizador Pro v36 — mensualidad azul, comparativa compacta, fix info simple');

console.log('✅ Cotizador Pro v37 — cotización simple state inicializado correctamente');

console.log('✅ Cotizador Pro v38 — separación opciones + términos compactos');

console.log('✅ Cotizador Pro v39 — footer comparativa ancho + cliente simple limpio');

console.log('✅ Cotizador Pro v40 — generador simple compacto, lógica original intacta');

console.log('✅ Cotizador Pro v41 — portabilidad/control/seguro compactos');

console.log('✅ Cotizador Pro v42 — tipografía Plus Jakarta aplicada a Vista previa');

console.log('✅ Cotizador Pro v43 — desglose mensual comparativa centrado');

console.log('✅ Cotizador Pro v44 — disclaimer comparativa ajustado al ancho');

console.log('✅ Cotizador Pro v45 — Roboto global en interfaz y cotizaciones');

console.log('✅ Cotizador Pro v46 — servicios del generador simple ultracompactos');

console.log('✅ Cotizador Pro v47 — fix real altura portabilidad y servicios');

console.log('✅ Cotizador Pro v48 — Roboto absoluta en toda la aplicación y generadores');

console.log('✅ Cotizador Pro v52 — tipografía Roboto limpia, sin filtros artificiales');

console.log('✅ Cotizador Pro v56 — prueba Roboto únicamente en botones del menú de escritorio');

console.log('✅ Cotizador Pro v57 — Roboto global + logo gris eliminado de cotizaciones');

console.log('✅ Cotizador Pro v58 — marca header alineada sólo escritorio');

console.log('✅ Cotizador Pro v59 — comparativa de características aislada y responsive');

console.log('✅ Cotizador Pro v60 — botón Características conectado al workspace real de escritorio');

console.log('✅ Cotizador Pro v61 — comparativa de características pulida + header escritorio alineado');

console.log('✅ Cotizador Pro v62 — cuerpo de comparativa visible y scroll corregido');

console.log('✅ Cotizador Pro v63 — scroll completo de comparativa en la pantalla principal');

console.log('✅ Cotizador Pro v64 — scroll comparativa con viewport explícito + jerarquía de botones invertida');

console.log('✅ Cotizador Pro v65 — fondo blanco en ficha de equipos de comparativa económica');

console.log('✅ Cotizador Pro v66 — importes del resumen alineados y contenidos');

console.log('✅ Cotizador Pro v67 — resumen restaurado al acomodo original');

console.log('✅ Cotizador Pro v68 — controles de comparación entre Bloque A y B sólo en móvil');

console.log('✅ Cotizador Pro v69 — bloque comparar ancho completo + separación móvil');

console.log('✅ Cotizador Pro v70 — cuadro completo de imagen blanco en Comparar equipos');

console.log('✅ Cotizador Pro v71 — fondo blanco aplicado al wrapper real de imagen en Comparar equipos');

console.log('✅ Cotizador Pro v72 — Características + gap móvil + Pago inicial alineado');

console.log('✅ Cotizador Pro v73 — Garantía');

console.log('✅ Cotizador Pro v75 — revert v74 + separación móvil segura sin tocar layout');

console.log('✅ Cotizador Pro v76 — botón Garantía');


// ═══ [v77] FUNNEL DE ACTIVIDAD: EQUIPO → PLAN → PLAZO ═════════════════════
// Usa las cotizaciones reales del período y después aplica el alcance oficial
// del usuario. No cambia los cálculos existentes del dashboard.
dashState.funnel = null;
dashState.funnelEquipo = null;
dashState.funnelPlan = null;

function _funnelCuenta(arr, keyFn){
  var m={};
  (arr||[]).forEach(function(x){
    var k=String(keyFn(x)||'Sin dato');
    m[k]=(m[k]||0)+1;
  });
  return Object.keys(m).map(function(k){return [k,m[k]];})
    .sort(function(a,b){return b[1]-a[1] || a[0].localeCompare(b[0]);});
}
function _funnelScope(c){
  var rol=_actividadRolOficial(asesorData&&asesorData.rol);
  if(rol==='ejecutivo') return String(c.attuid||'').toUpperCase()===String(asesorData.attuid||'').toUpperCase();
  if(rol==='gerente' || rol==='regional'){
    var ts=(asesorData.tiendasAsignadas&&asesorData.tiendasAsignadas.length)
      ? asesorData.tiendasAsignadas : [asesorData.tienda||asesorData.sucursal||''];
    if(dashState.tiendaFiltro) ts=[String(dashState.tiendaFiltro).replace(/_/g,' ')];
    return ts.map(String).indexOf(String(c.tienda||''))>=0;
  }
  if(rol==='nacional'){
    var regs=(typeof _misRegiones==='function')?_misRegiones():(asesorData.region?[asesorData.region]:[]);
    if(dashState.regionFiltro) regs=[dashState.regionFiltro];
    return regs.map(String).indexOf(String(c.region||''))>=0;
  }
  if(rol==='director' || rol==='admin'){
    if(dashState.regionFiltro && String(c.region||'')!==String(dashState.regionFiltro)) return false;
    if(dashState.tiendaFiltro && _kResumen(c.tienda||'')!==_kResumen(dashState.tiendaFiltro||'')) return false;
    return true;
  }
  return false;
}

async function cargarFunnelActividad(dias){
  await loadFirebase();
  if(!firestoreDB || !firestoreFns || !dias || !dias.length) return [];
  var desde=dias[0], hasta=dias[dias.length-1];
  var col=firestoreFns.collection(firestoreDB,'cotizaciones');
  var snap;
  try{
    // Una sola dimensión de rango: normalmente no requiere índice compuesto.
    snap=await firestoreFns.getDocs(firestoreFns.query(
      col,
      firestoreFns.where('fecha','>=',desde),
      firestoreFns.where('fecha','<=',hasta)
    ));
  }catch(e){
    console.warn('[Funnel] consulta por fecha no disponible:',e&&e.message);
    return [];
  }
  var rows=[], ids={};
  snap.forEach(function(docu){
    ...
    if(_funnelScope(d)){
      rows.push(d);
      if(d.id) ids[String(d.id)]=true;
      else ids[String(docu.id)]=true;
    }
  });

  // [v78] Para Ejecutivo, sumar temporalmente las cotizaciones pendientes de
  // la cola local. Así Equipo → Plan → Plazo aparece aunque la escritura esté
  // esperando sincronización. Al subir a Firestore se deduplica por id.
  if(_actividadRolOficial(asesorData&&asesorData.rol)==='ejecutivo'){
    try{
      var wanted={};
      dias.forEach(function(f){wanted[String(f)]=true;});
      (getColaCotizaciones()||[]).forEach(function(p){
        if(!p || !wanted[String(p.fecha||'')] || !_funnelScope(p)) return;
        var pid=String(p.id||'');
        if(pid && ids[pid]) return;
        rows.push(p);
        if(pid) ids[pid]=true;
      });
    }catch(_colaErr){}
...
function _funnelItemsHTML(items, tipo, selected){
  if(!items.length) return '<div class="dash-funnel-empty">Sin datos</div>';
  var max=items[0][1]||1;
  return items.slice(0,10).map(function(e){
    var val=e[0], n=e[1], pct=Math.max(4,Math.round(n/max*100));
    return '<button type="button" class="dash-funnel-item '+(selected===val?'on':'')+'" data-funnel="'+tipo+'" data-value="'+escapeHtml(val)+'">'+
      '<span class="dash-funnel-name">'+escapeHtml(val)+'</span><span class="dash-funnel-count">'+n.toLocaleString('es-MX')+'</span>'+
      '<span class="dash-funnel-bar"><i style="width:'+pct+'%"></i></span></button>';
  }).join('');
}

function renderFunnelActividad(){
  var host=document.getElementById('dash-funnel-host');
  if(!host) return;
  var all=dashState.funnel||[];
  if(!all.length){
    host.innerHTML='<div class="dash-section dash-funnel"><div class="dash-funnel-title">Equipos · Planes · Plazos</div><div class="dash-funnel-empty">No hay detalle disponible para este período.</div></div>';
    return;
  }
  var eq=dashState.funnelEquipo, pl=dashState.funnelPlan;
  var equipos=_funnelCuenta(all,function(x){return x.equipoNombre||x.equipoId||'Sin equipo';});
  var byEq=eq?all.filter(function(x){return String(x.equipoNombre||x.equipoId||'Sin equipo')===eq;}):all;
  var planes=_funnelCuenta(byEq,function(x){return x.plan||'Sin plan';});
  var byPlan=pl?byEq.filter(function(x){return String(x.plan||'Sin plan')===pl;}):byEq;
  var plazos=_funnelCuenta(byPlan,function(x){return x.plazo ? String(x.plazo)+' meses' : 'Sin plazo';});

  host.innerHTML='<div class="dash-section dash-funnel">'+
    '<div class="dash-funnel-head"><div><div class="dash-funnel-title">Equipos · Planes · Plazos</div>'+
    '<div class="dash-funnel-sub">'+all.length.toLocaleString('es-MX')+' cotizaciones del alcance actual · toca un equipo o plan para profundizar</div></div></div>'+
    '<div class="dash-funnel-grid">'+
      '<div class="dash-funnel-col"><div class="dash-funnel-col-title">1 · Equipos</div>'+_funnelItemsHTML(equipos,'equipo',eq)+'</div>'+
      '<div class="dash-funnel-col"><div class="dash-funnel-col-title">2 · Planes'+(eq?' de '+escapeHtml(eq):'')+'</div>'+_funnelItemsHTML(planes,'plan',pl)+'</div>'+
      '<div class="dash-funnel-col"><div class="dash-funnel-col-title">3 · Plazos'+(pl?' · '+escapeHtml(pl):'')+'</div>'+_funnelItemsHTML(plazos,'plazo',null)+'</div>'+
    '</div><div class="dash-funnel-note">El Excel incluye el detalle de las cotizaciones visibles según tu jerarquía y territorio.</div></div>';

  host.querySelectorAll('[data-funnel="equipo"]').forEach(function(b){
    b.addEventListener('click',function(){
      var v=b.getAttribute('data-value');
      dashState.funnelEquipo=(dashState.funnelEquipo===v)?null:v;
      dashState.funnelPlan=null;
      renderFunnelActividad();
    });
  });
  host.querySelectorAll('[data-funnel="plan"]').forEach(function(b){
    b.addEventListener('click',function(){
      var v=b.getAttribute('data-value');
      dashState.funnelPlan=(dashState.funnelPlan===v)?null:v;
      renderFunnelActividad();
    });
  });
}

async function montarFunnelActividad(data){
  var body=document.getElementById('dash-body');
  if(!body) return;
  var host=document.createElement('div');
  host.id='dash-funnel-host';
  body.appendChild(host);
  host.innerHTML='<div class="dash-section dash-funnel"><div class="dash-funnel-title">Equipos · Planes · Plazos</div><div class="dash-loading">Cargando detalle...</div></div>';
  dashState.funnelEquipo=null; dashState.funnelPlan=null;
  dashState.funnel=await cargarFunnelActividad((data.rango&&data.rango.dias)||[]);
  renderFunnelActividad();
}
