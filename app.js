...
(function(){
  function instalarBotonCompararEquipos(){
    ...
    btn.onclick=function(ev){
      ...
      // Abrimos el mismo modal normal para conservar toda la lógica existente.
      generar.click();

      // Ya dentro del modal, el usuario guarda esta configuración como Opción 1.
      setTimeout(function(){
        var b=document.getElementById('cot-device-compare-btn');
        if(b){
          b.scrollIntoView({behavior:'smooth',block:'center'});
          b.style.boxShadow='0 0 0 2px #36B9F2';
          setTimeout(function(){b.style.boxShadow='';},1400);
        }
      },180);
    };
  ...
  window.actualizarBotonCompararEquipos=function(){
    ...
    if(!btn) return;
    if(cotEquipoComparar && cotEquipoComparar.device){
      btn.textContent='OPCIÓN 1: '+cotEquipoComparar.device.name+' · ELEGIR OPCIÓN 2';
      if(clear) clear.style.display='';
    }else{
      btn.textContent='COMPARAR EQUIPOS';
      if(clear) clear.style.display='none';
