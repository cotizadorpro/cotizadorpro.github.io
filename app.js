...
async function doLogin(){
  ...
  try{
    ...
    if(!snap.exists()){
      errEl.textContent='Usuario no encontrado. Revísalo.';
      btn.disabled=false;btn.textContent='Entrar';
      return;
    }
    const emp=snap.data();
...
function openDiagnostico(){
  ...
  modal.appendChild(fila('Usuario', ad.attuid || '—', ad.attuid ? null : 'err'));
...
async function exportarDashboardExcel(){
  ...
  try {
    ...
    if(s.topAsesores && s.topAsesores.length){
      const rows = [['#', 'Usuario', 'Nombre', 'Tienda', 'Región', 'Cotizaciones', 'Estado']];
    ...
    if(data.regionales && data.regionales.length){
      const rows = [['#', 'Usuario', 'Nombre', 'Región', 'Tiendas', 'Cotizaciones', 'Estatus']];
...
// [v1.12.ROLE-A] Jerarquía oficial:
// ADMIN1 > Director > Nacional > Regional > Gerente > Ejecutivo.
// Compatibilidad histórica: asesor = ejecutivo; director_nacional = director.
const ADM_ROLES_OPERATIVOS = ['ejecutivo'];
const ADM_ROLES_MANDO = ['gerente', 'regional', 'nacional', 'director'];
const ADM_ROLE_LEVEL = { ejecutivo:1, gerente:2, regional:3, nacional:4, director:5 };
function _normalizarRol(r){
  r=String(r||'ejecutivo').trim().toLowerCase();
  if(r==='asesor') return 'ejecutivo';
  if(r==='director_nacional') return 'director';
  return r;
}
function _admRol(d){ return _normalizarRol(d && d.rol); }
function _admEsOperativo(d){ return _admRol(d)==='ejecutivo'; }
...
function _meRol(){ return _normalizarRol(asesorData && asesorData.rol); }
function _meRegion(){ return String((asesorData && asesorData.region) || ''); }
function _meTienda(){ return String((asesorData && (asesorData.tienda||asesorData.sucursal)) || ''); }
function _meTiendas(){
  const a=Array.isArray(asesorData && asesorData.tiendasAsignadas) ? asesorData.tiendasAsignadas.map(String).filter(Boolean) : [];
  if(a.length) return a;
  const t=_meTienda(); return t?[t]:[];
}
...
function _meNivel(){ return _meEsSuper()?99:(ADM_ROLE_LEVEL[_meRol()]||0); }

function esAdminAccesos(){
  if(!asesorData || !asesorData.attuid) return false;
  return _meEsSuper() || _meNivel() >= ADM_ROLE_LEVEL.gerente;
}
...
function _admHeroSub(){
  if(_meEsSuper()) return 'Administra toda la estructura de usuarios';
  const r = _meRol();
  if(r==='director') return 'Administra la estructura nacional';
  if(r==='nacional') return 'Administra las regiones de tu alcance';
  if(r==='regional') return 'Administra tus tiendas';
  if(r==='gerente') return 'Administra los ejecutivos de tu tienda';
...
function _admEnAlcance(d){
  ...
  const mr=_meRol(), dr=_admRol(d);
  if(mr==='nacional') return _misRegiones().indexOf(String(d.region||'')) >= 0;
  if(mr==='regional') return _meTiendas().indexOf(String(d.tienda||'')) >= 0 || (dr==='regional' && String(d.region||'')===_meRegion());
  if(mr==='gerente') return String(d.tienda||'')===_meTienda();
  return false;
}
function _admPermisos(attuid, d){
  ...
  if(esYo) return p;
  const targetRol=_admRol(d), targetNivel=ADM_ROLE_LEVEL[targetRol]||0;
  const puedeJerarquia=_meEsSuper() || (_meNivel()>targetNivel && _admEnAlcance(d));
  if(!puedeJerarquia) return p;
  p.toggle=true; p.eliminar=true; p.editar=true;
  if(targetRol==='ejecutivo' || targetRol==='gerente') p.mover=true;
  if(targetRol==='regional'){ p.tiendas=true; p.moverRegion=(_meEsSuper()||_meRol()==='director'||_meRol()==='nacional'); p.moverTienda=true; }
  return p;
}
...
function _admRoleTxt(d){
  const r = _admRol(d);
  return ({ejecutivo:'Ejecutivo',gerente:'Gerente',regional:'Regional',nacional:'Nacional',director:'Director'})[r] || r;
}
...
// ── Buscar por Usuario (campo interno attuid) ──
...
async function adminCargarEquipo(){
  if(!esAdminAccesos()) return;
  const scopeRegion=document.getElementById('adm-scope-region'), scopeBox=document.getElementById('adm-scope');
  const groups=document.getElementById('adm-groups'), listLabel=document.getElementById('adm-list-label');
  if(!groups) return;
  groups.innerHTML='<div class="adm-msg">Cargando…</div>';
  if(scopeRegion) scopeRegion.style.display='none'; if(scopeBox) scopeBox.style.display='none';
  try{
    await loadFirebase();
    const col=firestoreFns.collection(firestoreDB,'empleados');
    let docs=[];
    if(_meEsGlobal()){
      const qs=await firestoreFns.getDocs(col); qs.forEach(function(x){docs.push({id:x.id,d:x.data()||{}});});
    }else if(_meRol()==='nacional'){
      const regs=_misRegiones();
      for(let i=0;i<regs.length;i+=10){ const ch=regs.slice(i,i+10); const qs=await firestoreFns.getDocs(firestoreFns.query(col,firestoreFns.where('region','in',ch))); qs.forEach(function(x){docs.push({id:x.id,d:x.data()||{}});}); }
    }else if(_meRol()==='regional'){
      const tiendas=_meTiendas();
      for(let i=0;i<tiendas.length;i+=10){ const ch=tiendas.slice(i,i+10); const qs=await firestoreFns.getDocs(firestoreFns.query(col,firestoreFns.where('tienda','in',ch))); qs.forEach(function(x){docs.push({id:x.id,d:x.data()||{}});}); }
      // incluir al propio regional aunque no tenga tienda individual
      if(_meAttuid() && !docs.some(function(x){return x.id===_meAttuid();})){ const me=await firestoreFns.getDoc(firestoreFns.doc(firestoreDB,'empleados',_meAttuid())); if(me.exists()) docs.push({id:me.id,d:me.data()||{}}); }
    }else if(_meRol()==='gerente'){
      const t=_meTienda(); if(t){ const qs=await firestoreFns.getDocs(firestoreFns.query(col,firestoreFns.where('tienda','==',t))); qs.forEach(function(x){docs.push({id:x.id,d:x.data()||{}});}); }
    }
    const uniq={}; docs.forEach(function(x){uniq[x.id]=x;}); docs=Object.keys(uniq).map(function(k){return uniq[k];});
    _admGente={}; docs.forEach(function(x){_admGente[x.id]=x.d;});
    if(listLabel) listLabel.textContent=_meEsGlobal()?'Estructura nacional':'Tu estructura';
    const activas=docs.filter(function(x){return x.d.activo!==false;}).length;
    const tset={}, rset={}; docs.forEach(function(x){if(x.d.tienda)tset[String(x.d.tienda)]=1;if(x.d.region)rset[String(x.d.region)]=1;});
    if(scopeRegion){ scopeRegion.textContent=_meEsGlobal()?'Cobertura nacional':(_misRegiones().join(' + ')||_meTienda()||'—'); scopeRegion.style.display='block'; }
    if(scopeBox){ scopeBox.innerHTML=_admScopeCell(Object.keys(rset).length,'Regiones',false)+'<div class="adm-scope-div"></div>'+_admScopeCell(Object.keys(tset).length,'Tiendas',false)+'<div class="adm-scope-div"></div>'+_admScopeCell(activas,'Activas',true); scopeBox.style.display='flex'; }
    let h='';
    if(_meEsSuper()) h+='<button class="adm-add-btn" style="margin:2px 0 8px" onclick="admSheetCrearMando(\'director\')"><span class="adm-add-ic">+</span>Crear Director</button>';
    if(_meEsSuper()||_meRol()==='director') h+='<button class="adm-add-btn" style="margin:2px 0 8px" onclick="admSheetCrearMando(\'nacional\')"><span class="adm-add-ic">+</span>Crear Nacional</button>';
    if(_meEsSuper()||_meRol()==='director'||_meRol()==='nacional') h+='<button class="adm-add-btn" style="margin:2px 0 12px" onclick="admSheetCrearRegional()"><span class="adm-add-ic">+</span>Crear Regional</button>';
    const mandos=docs.filter(function(x){return _admEsMando(x.d);}).sort(function(a,b){return (ADM_ROLE_LEVEL[_admRol(b.d)]||0)-(ADM_ROLE_LEVEL[_admRol(a.d)]||0)||String(a.d.nombre||'').localeCompare(String(b.d.nombre||''));});
    if(mandos.length) h+=_admGrupoHTML('admg-mandos','Mandos',mandos);
    const ops=docs.filter(function(x){return !_admEsMando(x.d);}), porTienda={}, regByTienda={};
    ops.forEach(function(x){const t=String(x.d.tienda||'Sin tienda');(porTienda[t]=porTienda[t]||[]).push(x);if(!regByTienda[t]&&x.d.region)regByTienda[t]=String(x.d.region);});
    let gi=0; Object.keys(porTienda).sort().forEach(function(t){h+=_admGrupoHTML('admg-'+(gi++),t,porTienda[t],t,regByTienda[t]);});
    if((_meRol()==='regional'||_meRol()==='gerente') && _meTienda() && !porTienda[_meTienda()]) h+=_admGrupoHTML('admg-propia',_meTienda(),[],_meTienda(),_meRegion());
    groups.innerHTML=h||'<div class="adm-msg">No hay colaboradores en tu alcance todavía.</div>';
  }catch(e){ groups.innerHTML='<div class="adm-msg adm-err">No se pudo cargar tu estructura. Reintenta.</div>'; console.log('[Accesos] error:',e&&e.message); }
}
...
function admSheetCrear(tienda, region){
  ...
  admAbrirSheet(
    ...
    +'<div class="adm-label" style="margin:16px 2px 8px">Usuario <span style="font-weight:400;text-transform:none;color:var(--hv2-ink3)">- en MAYUSCULAS</span></div>'
    ...
    +'<div class="adm-label" style="margin:18px 2px 8px">¿Es Ejecutivo o Gerente?</div>'
    +'<div class="adm-opt" data-v="ejecutivo" onclick="admSelUnico(this)">'+tick+'<span class="adm-opt-name">Ejecutivo</span></div>'
    +'<div class="adm-opt" data-v="gerente" onclick="admSelUnico(this)">'+tick+'<span class="adm-opt-name">Gerente</span></div>'
    +'<div style="margin:16px 2px 2px;padding:11px 13px;background:var(--hv2-accent-soft);border:.5px solid rgba(56,138,221,.28);border-radius:12px;font-size:12.5px;line-height:1.55;color:var(--hv2-ink2)">La contrase&ntilde;a inicial ser&aacute; el mismo <b>Usuario</b> (en may&uacute;sculas). El asesor entra con su Usuario como usuario y como contrase&ntilde;a.</div>'
...
async function admConfirmCrear(){
  ...
  const rol = (_admSel==='gerente') ? 'gerente' : (_admSel==='ejecutivo' ? 'ejecutivo' : '');
  if(attuid.length < 4 || !/^[A-Z0-9]+$/.test(attuid)){ admToast('Usuario inválido: solo mayúsculas y números.','err'); return; }
  ...
  try{
    ...
    const snap = await firestoreFns.getDoc(ref);
    if(snap.exists()){
      admToast('Ya existe un colaborador con Usuario '+attuid+'.','err');
    ...
    admCerrarSheet();
    admToast((rol==='gerente'?'Gerente':'Ejecutivo')+' creado - '+attuid,'ok');
    _admCrearCtx = null;
    adminCargarEquipo();
  }catch(e){
...
// [v1.12.ROLE-A] -- Crear Director / Nacional ---------------------------------
let _admCrearMandoRol=null;
function admSheetCrearMando(rol){
  rol=_normalizarRol(rol);
  if(!esAdminAccesos() || (rol!=='director' && rol!=='nacional')) return;
  if(rol==='director' && !_meEsSuper()){ admToast('Solo ADMIN1 puede crear Directores.','err'); return; }
  if(rol==='nacional' && !(_meEsSuper()||_meRol()==='director')){ admToast('No tienes permiso para crear Nacionales.','err'); return; }
  _admCrearMandoRol=rol;
  const titulo=rol==='director'?'Director':'Nacional';
  const regs=_meEsGlobal()?((typeof REGIONES_MX!=='undefined')?REGIONES_MX.slice():[]):_misRegiones();
  let opts='';
  if(rol==='nacional') regs.forEach(function(r){opts+='<div class="adm-opt" data-v="'+_admEsc(r)+'" onclick="this.classList.toggle(\'sel\')"><span class="adm-opt-tick">✓</span><span class="adm-opt-name">'+_admEsc(r)+'</span></div>';});
  admAbrirSheet('<div class="adm-sheet-h">Crear '+titulo+'</div>'
    +'<div class="adm-sheet-sub">Alta de usuario con rango <b>'+titulo+'</b>.</div>'
    +'<div class="adm-label" style="margin:16px 2px 8px">Usuario <span style="font-weight:400;text-transform:none;color:var(--hv2-ink3)">- en MAYÚSCULAS</span></div>'
    +'<input id="adm-new-attuid" class="adm-pass-in" type="text" placeholder="Ej. NACIONAL_A" autocapitalize="characters" autocomplete="off" oninput="this.value=this.value.toUpperCase()">'
    +'<div class="adm-label" style="margin:16px 2px 8px">Nombre</div><input id="adm-new-nombre" class="adm-pass-in" type="text" placeholder="Ej. NACIONAL" autocapitalize="characters" autocomplete="off" oninput="this.value=this.value.toUpperCase()">'
    +(rol==='nacional'?'<div class="adm-label" style="margin:18px 2px 8px">Regiones asignadas</div><div class="adm-sheet-scroll" id="adm-new-regiones">'+(opts||'<div class="adm-msg">Primero configura las regiones.</div>')+'</div>':'')
    +'<div style="margin:16px 2px 2px;padding:11px 13px;background:var(--hv2-accent-soft);border:.5px solid rgba(56,138,221,.28);border-radius:12px;font-size:12.5px;line-height:1.55;color:var(--hv2-ink2)">La contraseña inicial será el mismo <b>Usuario</b>.</div>'
    +'<button class="adm-sheet-cta" id="adm-cta" onclick="admConfirmCrearMando()">Crear '+titulo+'</button>');
}
async function admConfirmCrearMando(){
  const rol=_admCrearMandoRol, fa=document.getElementById('adm-new-attuid'), fn=document.getElementById('adm-new-nombre');
  const attuid=(fa?String(fa.value||''):'').toUpperCase().replace(/\s+/g,''), nombre=fn?String(fn.value||'').trim():'';
  if(attuid.length<4 || !/^[A-Z0-9_]+$/.test(attuid)){admToast('Usuario inválido: usa mayúsculas, números o guion bajo.','err');return;}
  if(!nombre){admToast('Escribe el nombre.','err');return;}
  const regiones=[]; const rb=document.getElementById('adm-new-regiones'); if(rb) Array.prototype.forEach.call(rb.querySelectorAll('.adm-opt.sel'),function(o){regiones.push(o.getAttribute('data-v'));});
  if(rol==='nacional'&&!regiones.length){admToast('Asigna al menos una región.','err');return;}
  const cta=document.getElementById('adm-cta'); if(cta){cta.disabled=true;cta.textContent='Creando...';}
  try{
    await loadFirebase(); const ref=firestoreFns.doc(firestoreDB,'empleados',attuid), snap=await firestoreFns.getDoc(ref);
    if(snap.exists()){admToast('Ya existe un colaborador con Usuario '+attuid+'.','err');if(cta){cta.disabled=false;cta.textContent='Crear';}return;}
    const data={attuid:attuid,nombre:nombre,rol:rol,activo:true,password:attuid,creadoPor:_meAttuid(),creadoTs:firestoreFns.serverTimestamp()};
    if(rol==='nacional'){data.regionesAsignadas=regiones;data.region=regiones[0]||'';}
    await firestoreFns.setDoc(ref,data); admCerrarSheet(); admToast((rol==='director'?'Director':'Nacional')+' creado - '+attuid,'ok'); _admCrearMandoRol=null; adminCargarEquipo();
  }catch(e){const msg=(e&&e.message)?e.message:'No se pudo crear.';admToast(/permission|insufficient/i.test(msg)?'Sin permiso para crear: hay que actualizar las reglas de Firestore para la nueva jerarquía.':msg,'err');if(cta){cta.disabled=false;cta.textContent='Crear';}}
}
...
function admSheetCrearRegional(){
  ...
  admAbrirSheet(
    ...
    +'<div class="adm-label" style="margin:4px 2px 8px">Usuario <span style="font-weight:400;text-transform:none;color:var(--hv2-ink3)">- en MAYUSCULAS</span></div>'
    ...
    + opts
    +'<div style="margin:16px 2px 2px;padding:11px 13px;background:var(--hv2-accent-soft);border:.5px solid rgba(56,138,221,.28);border-radius:12px;font-size:12.5px;line-height:1.55;color:var(--hv2-ink2)">La contrase&ntilde;a inicial ser&aacute; el mismo <b>Usuario</b>. Empieza sin tiendas; asignaselas despues con "Editar tiendas".</div>'
    +'</div>'
...
async function admConfirmCrearRegional(){
  ...
  if((_meRol()==='nacional') && _misRegiones().indexOf(region)<0){ admToast('Esa region no esta en tu alcance.','err'); return; }
  const cta=document.getElementById('adm-cta'); let lbl=''; if(cta){ lbl=cta.textContent; cta.disabled=true; cta.textContent='Creando...'; }
  try{
    await loadFirebase();
    const ref=firestoreFns.doc(firestoreDB,'empleados',attuid);
    const snap=await firestoreFns.getDoc(ref);
    if(snap.exists()){ admToast('Ya existe un colaborador con Usuario '+attuid+'.','err'); if(cta){cta.disabled=false;cta.textContent=lbl||'Crear regional';} return; }
