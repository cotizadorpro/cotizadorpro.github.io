...
function openCotModal(){
  ...
  // Build rentas pills (0,1,2,3) + [v1.8.2] botón "Dep. Garantía"
  ...
  rentasHtml += '<button class="cot-pill cot-pill-custom" onclick="cotSetDeposito()" id="cot-deposito-pill">Dep. Garantía</button>';
...
const controlPrice = cotState.control ? 50 : 0;
  ...
  // [v1.8.2] Dep. Garantía (se suma al pago inicial, NO disminuye mensualidad)
  if(depositoGarantia > 0){
    h += '<div class="cot-resumen-row"><span class="cot-resumen-label">Dep. Garantía</span><span class="cot-resumen-val">$'+fmx(depositoGarantia)+'</span></div>';
  }
...
function cotSend(){
  ...
  const engPay = cotCalcEngancheAmount();
  const rentasGarantia = cotState.rentas * cotState.planRenta;
  // [v1.8.3] Dep. Garantía: suma al pago inicial
  const depositoGarantia = cotState.deposito || 0;
  const totalInicial = engPay + rentasGarantia + depositoGarantia;
...
const totalMensualSinPort =
 ...
 if(cotState.port){
  ...
  if(depositoGarantia > 0) _conceptosIniciales.push('Dep. Garantía $' + fmx(depositoGarantia));
...
console.log('✅ Cotizador Pro v73 — Dep. Garantía');
