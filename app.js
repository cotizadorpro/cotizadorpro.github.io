...
function cotStateDesdeSeleccionPrincipal(){
  ...
  var dev=all.find(function(d){return d.id===curFichaId;});
  var info=PRICES[curFichaId];
  if(!dev || !info) return null;

  var promo=resolvePrice(curFichaId,curPlan,curPlazo);
  if(promo===null || promo===undefined) return null;

  var RENTAS={'Azul 1':330,'Azul 2':435,'Azul 3':550,'Plata':650,'Oro':725,'Black':825,'Platino':1035,'Diamante':1300,'Titanio':1599};
  var renta=RENTAS[curPlan]||0;
  var desc=info.contado ? Math.round((1-promo/info.contado)*100) : 0;

  var bid=curFichaId;
  for(var k in (window.STORAGE_VARIANTS||{})){
    if(((window.STORAGE_VARIANTS[k]||[]).some(function(v){return v[1]===curFichaId;}))){
      bid=k; break;
    }
  }
  var fp=(window.FUTURE_PRICES||{})[bid] || (window.FUTURE_PRICES||{})[curFichaId];
  var upcoming=Array.isArray(window.UPCOMING_ONLY) &&
    (window.UPCOMING_ONLY.indexOf(bid)>=0 || window.UPCOMING_ONLY.indexOf(curFichaId)>=0);
...
// [v1.11.65] BOTÓN COMPARAR EQUIPOS EN COTIZADOR PRINCIPAL
(function(){
  function instalarBotonCompararEquipos(){
    if(document.getElementById('cot-main-device-compare-btn')) return;

    var buttons=Array.prototype.slice.call(document.querySelectorAll('button'));
    var generar=buttons.find(function(b){
      return /generar cotizaci[oó]n/i.test((b.textContent||'').trim()) &&
             !/imagen/i.test((b.textContent||'').trim());
    });
    if(!generar) return;

    var wrap=document.createElement('div');
    wrap.id='cot-main-device-compare-wrap';
    wrap.style.cssText='display:flex;gap:8px;margin:0 0 8px;width:100%';

    var btn=document.createElement('button');
    btn.type='button';
    btn.id='cot-main-device-compare-btn';
    btn.className=generar.className;
    btn.style.cssText='flex:1;background:#59616A!important;border:1px solid rgba(255,255,255,.18)!important;color:#fff!important;min-height:36px';
    btn.textContent='COMPARAR EQUIPOS';
    btn.onclick=async function(ev){
      ev.preventDefault();
      ev.stopPropagation();

      var actual=cotStateDesdeSeleccionPrincipal();
      if(!actual){
        alert('Selecciona un equipo, plan y plazo para comparar.');
        return;
      }

      // PRIMER CLIC: guardar equipo 1. No abre ningún modal.
      if(!cotEquipoComparar || !cotEquipoComparar.device){
        cotEquipoComparar=JSON.parse(JSON.stringify(actual));
        actualizarBotonCompararEquipos();
        alert('Opción 1 guardada: '+actual.device.name+'. Ahora selecciona el segundo equipo.');
        return;
      }

      // Evitar comparar accidentalmente exactamente la misma selección.
      ...
      // SEGUNDO CLIC: generar directamente el flyer Bloque 1 / Bloque 2.
    ...
    // Si ya existe una Opción 1, mostrar también acceso para limpiarla.
    var clear=document.createElement('button');
    clear.type='button';
    clear.id='cot-main-device-compare-clear';
    clear.textContent='×';
    clear.title='Cancelar comparativa';
    clear.style.cssText='display:none;width:40px;min-height:36px;border-radius:8px;border:1px solid rgba(255,255,255,.18);background:#59616A;color:#fff;font-size:20px;cursor:pointer';
    clear.onclick=function(e){
      e.preventDefault(); e.stopPropagation();
      cotClearDeviceCompare();
      actualizarBotonCompararEquipos();
    };
    wrap.appendChild(clear);

    // Colócalo en la esquina superior derecha del bloque del cotizador
    // sin ocupar espacio en el flujo ni modificar la altura del panel.
    var panel=generar.closest('.card, .panel, .detail, .quote, section') || generar.parentElement;
    if(panel){
      var ps=getComputedStyle(panel).position;
      if(ps==='static') panel.style.position='relative';
      ...
      generar.parentNode.insertBefore(wrap,generar);
    }
    actualizarBotonCompararEquipos();
  }

  window.actualizarBotonCompararEquipos=function(){
    var btn=document.getElementById('cot-main-device-compare-btn');
    var clear=document.getElementById('cot-main-device-compare-clear');
    if(!btn) return;
    if(cotEquipoComparar && cotEquipoComparar.device){
      btn.textContent='GENERAR COMPARATIVA';
      if(clear) clear.style.display='';
    }else{
      btn.textContent='COMPARAR EQUIPOS';
      if(clear) clear.style.display='none';
  ...
  document.addEventListener('DOMContentLoaded',function(){
    setTimeout(instalarBotonCompararEquipos,250);
    setTimeout(instalarBotonCompararEquipos,900);
  });
  window.addEventListener('load',function(){setTimeout(instalarBotonCompararEquipos,150);});

  // La pantalla del cotizador se reconstruye en algunos flujos: reintento ligero.
  setInterval(instalarBotonCompararEquipos,1500);
})();
