...
function showFicha(id){
  ...
  h+='<button id="cot-main-device-compare-btn" type="button" onclick="compararEquipoActual()" style="position:absolute;right:14px;top:8px;z-index:6;min-height:32px;padding:0 14px;border-radius:8px;border:1px solid rgba(255,255,255,.14);background:#59616A;color:#fff;font-size:10px;font-weight:700;letter-spacing:.25px;cursor:pointer">'+((cotEquipoComparar&&cotEquipoComparar.device)?'GENERAR COMPARATIVA':'COMPARAR EQUIPOS')+'</button>';
...
// [v1.11.67] COMPARATIVA NATIVA DE 2 EQUIPOS.
// Vive en app.js para compartir el mismo scope de curFichaId/curPlan/curPlazo
// que usa el generador normal. No depende de leer clases del DOM.
function _capturarEquipoComparativa(){
  if(!curFichaId || !curPlan || !curPlazo) return null;
  ...
  const desc=info.contado ? (1-promo/info.contado)*100 : 0;

  let _bid=curFichaId;
  for(const bid in (window.STORAGE_VARIANTS||{})){
    if((window.STORAGE_VARIANTS[bid]||[]).some(function(v){return v[1]===curFichaId;})){
      _bid=bid; break;
    }
  }
  const fp=(window.FUTURE_PRICES||{})[_bid] || (window.FUTURE_PRICES||{})[curFichaId];
  const isUpcoming=Array.isArray(window.UPCOMING_ONLY) &&
    (window.UPCOMING_ONLY.indexOf(_bid)>=0 || window.UPCOMING_ONLY.indexOf(curFichaId)>=0);
...
async function compararEquipoActual(){
  const actual=_capturarEquipoComparativa();
  if(!actual){
    alert('Selecciona un equipo, plan y plazo antes de comparar.');
    return;
  }

  if(!cotEquipoComparar || !cotEquipoComparar.device){
    cotEquipoComparar=JSON.parse(JSON.stringify(actual));
    const b=document.getElementById('cot-main-device-compare-btn');
    if(b) b.textContent='GENERAR COMPARATIVA';
    alert('Opción 1 guardada: '+actual.device.name+'. Ahora selecciona el segundo equipo.');
    return;
  }

  if(cotEquipoComparar.device.id===actual.device.id &&
     cotEquipoComparar.plan===actual.plan &&
     String(cotEquipoComparar.plazo)===String(actual.plazo)){
    alert('Selecciona otro equipo o una configuración diferente para la Opción 2.');
    return;
  }

  cotState=JSON.parse(JSON.stringify(actual));
  await generateFlyerImage();
}

function cancelarComparativaEquipos(){
  cotEquipoComparar=null;
  const b=document.getElementById('cot-main-device-compare-btn');
  if(b) b.textContent='COMPARAR EQUIPOS';
}
...
// [v1.11.64] Refresca el bloque de comparativa dentro del modal.
function cotRefreshDeviceCompareUI(){
  var box=document.getElementById('cot-device-compare-box');
  var btn=document.getElementById('cot-device-compare-btn');
  var clear=document.getElementById('cot-device-compare-clear');
  if(!box||!btn) return;

  if(cotEquipoComparar && cotEquipoComparar.device){
    box.style.display='block';
    box.innerHTML='<b>Opción 1 guardada:</b> '+cotEquipoComparar.device.name+
      ' · '+cotEquipoComparar.plan+' · '+cotEquipoComparar.plazo+' meses'+
      '<br><span style="opacity:.72">Configura este equipo como Opción 2 y después usa “Enviar cotización como imagen”.</span>';
    btn.textContent='Reemplazar Opción 1 con este equipo';
    if(clear) clear.style.display='';
  }else{
    box.style.display='none';
    box.innerHTML='';
    btn.textContent='Guardar como Opción 1 y elegir otro equipo';
    if(clear) clear.style.display='none';
  }
}

function cotClearDeviceCompare(){
  cotEquipoComparar=null;
  cotRefreshDeviceCompareUI();
  if(typeof actualizarBotonCompararEquipos==='function') actualizarBotonCompararEquipos();
}

function cotSaveDeviceCompare(){
  if(!cotState || !cotState.device){
    alert('No hay una cotización para guardar.');
    return;
  }
  if(!cotValidarTipo()) return;

  // Asegura que montos manuales escritos en inputs queden reflejados.
  if(typeof cotRender==='function') cotRender();

  cotEquipoComparar=JSON.parse(JSON.stringify(cotState));
  cotEquipoComparar.comparar=[]; // la comparativa nueva es entre equipos, no entre planes
  if(typeof actualizarBotonCompararEquipos==='function') actualizarBotonCompararEquipos();

  var nombre=cotEquipoComparar.device.name;
  cotClose();

  // Regresa al catálogo para escoger el segundo equipo.
  try{ show('s-catalog'); }catch(e){}

  setTimeout(function(){
    alert('Opción 1 guardada: '+nombre+'. Ahora selecciona el segundo equipo, configura su plan y pulsa “Enviar cotización como imagen”.');
  },80);
}
...
const isUpcoming=
  ...
  if(typeof cotRefreshDeviceCompareUI==='function') cotRefreshDeviceCompareUI();
...
function buildFlyerTwoDevicesHTML(a,b){
  function calc(s){
    ...
    return {eng:eng,rentas:rentas,deposito:deposito,inicial:inicial,seguro:seguro,
            control:control,eq:eq,mensual:mensual,mensualPort:mensualPort,desc:desc};
  }
  function card(s,n){
    var c=calc(s);
    var accent=FLYER_PLAN_ACCENTS[s.plan]||'#00A8E0';
    var img=(IMG&&IMG[s.device.id])
      ? '<img src="'+IMG[s.device.id]+'" alt="" class="flyer-devcmp-img">'
      : '<div class="flyer-devcmp-img-fallback">📱</div>';
    var h='<div class="flyer-devcmp-card" style="--cmp-accent:'+accent+'">';
    h+='<div class="flyer-devcmp-option">OPCIÓN '+n+'</div>';
    h+='<div class="flyer-devcmp-product">'+img;
    h+='<div class="flyer-devcmp-product-meta"><div class="flyer-devcmp-brand">'+s.device.brand+'</div>';
    h+='<div class="flyer-devcmp-name">'+s.device.name+'</div>';
    h+='<div class="flyer-devcmp-storage">'+s.device.storage+'</div></div></div>';
    h+='<div class="flyer-devcmp-plan"><b>'+s.plan+'</b><span>'+s.plazo+' meses · renta $'+_flyerFmx(s.planRenta)+'</span></div>';
    h+='<div class="flyer-devcmp-row"><span>Precio equipo</span><b>'+(s.promo===0?'Incluido*':'$'+_flyerFmx(Math.round(s.promo)))+'</b></div>';
    if(c.desc>0 && s.promo>0) h+='<div class="flyer-devcmp-discount">−'+c.desc+'% de descuento</div>';
    h+='<div class="flyer-devcmp-row"><span>Pago inicial</span><b>$'+_flyerFmx(c.inicial)+'</b></div>';
    h+='<div class="flyer-devcmp-month"><span>Mensualidad</span><b>$'+_flyerFmx(c.mensual)+'/mes</b></div>';
    var det=[];
    det.push('$'+_flyerFmx(s.planRenta)+' plan');
    ...
    h+='<div class="flyer-devcmp-detail">'+det.join(' + ')+'</div>';
    if(s.port){
      h+='<div class="flyer-devcmp-port">'+(s.plan==='Titanio'?'10':'20')+'% portabilidad · <b>$'+
        _flyerFmx(c.mensualPort)+'/mes</b><br>facturas 2 a 7</div>';
    }
    h+='</div>';
    return h;
  }

  var stateForGreeting=b||a;
  // Conserva el nombre del cliente de cualquiera de las dos opciones.
  if(!stateForGreeting.cliente && a.cliente) stateForGreeting.cliente=a.cliente;

  var h='<div class="flyer-v3 flyer-v3-device-compare">';
  h+=_flyerHead();
  h+=_flyerGreet(stateForGreeting);
  h+='<div class="flyer-devcmp-title">Comparativa de equipos</div>';
  h+='<div class="flyer-devcmp-grid">'+card(a,1)+card(b,2)+'</div>';
