...
function buildSpecsPDFHTML(dev){
  ...
  return '<div class="specs-page" style="width:794px;min-height:1123px;background:#FFFFFF;font-family:-apple-system,BlinkMacSystemFont,\'Segoe UI\',Inter,sans-serif;color:#26313D;line-height:1.35">'+
    // ── Header: marca + fecha ────────────────────────────────────────────
'<div style="display:flex;align-items:center;justify-content:space-between;padding:30px 42px 22px;background:linear-gradient(135deg,#26384F 0%,#1B4F8A 100%);border-bottom:1px solid #20344C;color:#fff">'+
  '<div style="display:flex;align-items:baseline;gap:0">'+
    '<span style="font-size:26px;font-weight:700;color:#FFFFFF;letter-spacing:-0.02em">COT</span>'+
    '<span style="font-size:26px;font-weight:300;color:#73C8EF;margin:0 2px">|</span>'+
    '<span style="font-size:26px;font-weight:700;color:#FFFFFF;letter-spacing:-0.02em">ZADOR</span>'+
    '<span style="font-size:14px;font-weight:500;color:#D7E0E8;margin-left:4px;letter-spacing:0.08em">PRO</span>'+
  '</div>'+
  '<div style="font-size:11px;color:#D7E0E8;letter-spacing:0.1em;font-weight:600">FICHA TÉCNICA · '+dateStr+'</div>'+
'</div>'+
...
async function generateSpecsPDF(){
  ...
  try {
    ...
    // A4 real: ocupar siempre el ancho completo de la hoja.
    // Si el contenido supera una página, continúa en otra sin crear márgenes laterales.
...
function _actividadActualizarContexto(){
  try{
    ...
    if(n){
      var key=(typeof fechaToISO==='function')?fechaToISO(now):'';
      var count=0;
      try{ count=Number(localStorage.getItem('primemx_cot_hoy_'+key)||0)||0; }catch(e){}
      // El contador real se refresca desde /resumenes al cargar el dashboard.
...
function _flyerFooter(discExtra, conRedes){
  ...
  if(typeof asesorData !== 'undefined' && asesorData && asesorData.name){
    ...
    h+='</div>';
    h+='<img class="flyer-v3-cotizador-logo" src="icon-192.png" alt="Cotizador Pro" crossorigin="anonymous">';
    h+='</div>';
  }
