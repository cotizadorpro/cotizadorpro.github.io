...
function buildSpecsPDFHTML(dev){
  ...
  return '<div class="specs-page" style="width:794px;height:1123px;overflow:hidden;background:#FFFFFF;font-family:-apple-system,BlinkMacSystemFont,\'Segoe UI\',Inter,sans-serif;color:#26313D;line-height:1.35">'+
    // ── Header: identidad Cotizador Pro del index + fecha ────────────────
...
'</div>'+
    ...
    '<div style="padding:0 42px 24px">'+
      '<div style="font-size:11px;font-weight:700;color:#1B4F8A;text-transform:uppercase;letter-spacing:0.12em;margin-bottom:12px">Especificaciones</div>'+
...
async function generateSpecsPDF(){
  ...
  try {
    ...
    // Convertir la ficha completa al aspecto A4 exacto: una sola página,
    // sin segunda hoja y sin franjas blancas laterales.
    const a4Canvas = document.createElement('canvas');
    a4Canvas.width = 1588;
    a4Canvas.height = 2246;
    const a4ctx = a4Canvas.getContext('2d');
    a4ctx.fillStyle = '#FFFFFF';
    a4ctx.fillRect(0,0,a4Canvas.width,a4Canvas.height);
    a4ctx.drawImage(canvas,0,0,canvas.width,canvas.height,0,0,a4Canvas.width,a4Canvas.height);
    const imgData = a4Canvas.toDataURL('image/jpeg',0.94);
...
function _flyerFooter(discExtra, conRedes){
  ...
  if(typeof asesorData !== 'undefined' && asesorData && asesorData.name){
    ...
    h+='</div>';
    h+='<img class="flyer-v3-cotizador-logo" src="cotizador-pro-logo-transparente.png" alt="Cotizador Pro" crossorigin="anonymous">';
    h+='</div>';
  }
